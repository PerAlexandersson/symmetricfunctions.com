...
\begin{polydata}{testFamily}
  ...
  Bib    & Jacobi1841 \\
  Year   & 1841 \\
  PositiveIn  & testFamily | Jacobi1841 \\
  Contains    & testFamily \\
  Generalizes & testFamily [Jacobi1841] \\
  Generalizes & testFamily; testFamily | Jacobi1841 \\
  SpecializesTo & testFamily | Jacobi1841,Cauchy1815 | map=q=0; status=conjecture; note=unit test \\
\end{polydata}
...
\textbf{En dashes:} Murnaghan--Nakayama.
...
There is a file link: \filelink{files/jack-rook-hit-data.txt}{jack-rook-hit-data.txt}.
Raw angle brackets in math are normalized: $i<j$ and \(j>i\).

\begin{tabular}{ll}
Table link & \hyperref[testFamily]{table link}\\
\end{tabular}

\begin{tabular}{lll}
Mixed math & \textbf{bold} & Chain \(1\lt 2\lt 3\)\\
\end{tabular}
