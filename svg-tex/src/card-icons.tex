...
\tikzsetnextfilename{card-polytope}
\begin{tikzpicture}[icon]
  \cardframe

  % Coordinates for Tesseract Projection
  % Outer square
  \coordinate (O_TL) at (-0.65, 0.65);
  \coordinate (O_TR) at (0.65, 0.65);
  \coordinate (O_BR) at (0.65, -0.65);
  \coordinate (O_BL) at (-0.65, -0.65);

  % Inner square
  \coordinate (I_TL) at (-0.25, 0.25);
  \coordinate (I_TR) at (0.25, 0.25);
  \coordinate (I_BR) at (0.25, -0.25);
  \coordinate (I_BL) at (-0.25, -0.25);

  % --- Fills (Depth cues similar to card-planePartitions) ---
  % Top face (Lightest)
  \fill[iconblue!20] (O_TL) -- (O_TR) -- (I_TR) -- (I_TL) -- cycle;
  % Left face (Medium)
  \fill[iconblue!35] (O_TL) -- (O_BL) -- (I_BL) -- (I_TL) -- cycle;
  % Right face (Medium)
  \fill[iconblue!35] (O_TR) -- (O_BR) -- (I_BR) -- (I_TR) -- cycle;
  % Bottom face (Darkest)
  \fill[iconblue!60] (O_BL) -- (O_BR) -- (I_BR) -- (I_BL) -- cycle;

  % --- Wireframe Lines ---
  % Outer Square
  \draw[black] (O_TL) -- (O_TR) -- (O_BR) -- (O_BL) -- cycle;
  % Inner Square
  \draw[black] (I_TL) -- (I_TR) -- (I_BR) -- (I_BL) -- cycle;
  % Connecting Edges
  \draw[black] (O_TL) -- (I_TL);
  \draw[black] (O_TR) -- (I_TR);
  \draw[black] (O_BR) -- (I_BR);
  \draw[black] (O_BL) -- (I_BL);

  % --- Vertices (Optional, following card-matroid style) ---
  % Inner vertices (Blue)
  \node[node-blue] at (I_TL) {};
  \node[node-blue] at (I_TR) {};
  \node[node-blue] at (I_BR) {};
  \node[node-blue] at (I_BL) {};
...
\tikzsetnextfilename{card-qAnalogs}
\begin{tikzpicture}[icon]
  \cardframe
...
\tikzsetnextfilename{card-rootSystem}
\begin{tikzpicture}[icon]
  \cardframe
...
\tikzsetnextfilename{card-tableauOperations}
\begin{tikzpicture}[icon]
  \cardframe
  \def\b{0.55}
...
\tikzsetnextfilename{card-schubertCalculus}
\begin{tikzpicture}[icon]
  \cardframe

  % A small Schubert-calculus icon: a 2x3 Grassmannian rectangle with
  % two incidence lines meeting in a highlighted Schubert cell.
  \begin{scope}[xscale=0.34,yscale=0.34,shift={(-1.55,-1.05)}]
    % 2 by 3 rectangle of Schubert boxes
    \foreach \x in {0,1,2} {
      \foreach \y in {0,1} {
    ...
    % Partition (2,1)
    \filldraw[iconblue, draw=black] (0,1) rectangle ++(1,1);
    \filldraw[iconblue, draw=black] (1,1) rectangle ++(1,1);
    \filldraw[iconblue, draw=black] (0,0) rectangle ++(1,1);

    % Incidence lines
    \draw[black, thinner] (-0.15,-0.15) -- (3.15,2.15);
    \draw[black, thinner] (-0.15,2.15) -- (3.15,-0.15);

    % Intersection point
    \node[node-blue] at (1.5,1) {};
  \end{scope}
\end{tikzpicture}
...
\tikzsetnextfilename{card-realRootedWords}
\begin{tikzpicture}[icon]
  \cardframe

  % Baseline positions for a permutation statistic.
  \draw[black!45, thinner] (-0.72,-0.62) -- (0.72,-0.62);
  \foreach \x in {-0.72,-0.36,0,0.36,0.72} {
    \draw[black!45, thinner] (\x,-0.69) -- (\x,-0.55);
  }

  % Permutation graph for 3,1,4,2,5 with descents emphasized in blue.
  \coordinate (A) at (-0.72, 0.10);
  \coordinate (B) at (-0.36,-0.26);
  \coordinate (C) at ( 0.00, 0.44);
  \coordinate (D) at ( 0.36,-0.02);
  \coordinate (E) at ( 0.72, 0.62);
  ...
  % Descent positions along the word.
  \node[node-blue, minimum size=4.5pt] at (-0.72,-0.62) {};
  \node[node-blue, minimum size=4.5pt] at (0,-0.62) {};
\end{tikzpicture}


\tikzsetnextfilename{card-realRootedTableaux}
\begin{tikzpicture}[icon]
  \cardframe

  % A Ferrers diagram extruded into a simple polytope silhouette.
  \begin{scope}[line width=0.85mm]
    % Front tableau vertices
    \coordinate (P1) at (-0.72, 0.46);
    \coordinate (P2) at ( 0.12, 0.46);
    \coordinate (P3) at ( 0.12, 0.18);
    \coordinate (P4) at (-0.16, 0.18);
    \coordinate (P5) at (-0.16,-0.10);
    \coordinate (P6) at (-0.44,-0.10);
    \coordinate (P7) at (-0.44,-0.38);
    \coordinate (P8) at (-0.72,-0.38);

    % Back polytope face
    \coordinate (Q1) at (-0.46, 0.66);
    \coordinate (Q2) at ( 0.38, 0.66);
    \coordinate (Q3) at ( 0.38, 0.38);
    \coordinate (Q4) at ( 0.10, 0.38);
    \coordinate (Q5) at ( 0.10, 0.10);
    \coordinate (Q6) at (-0.18, 0.10);
    \coordinate (Q7) at (-0.18,-0.18);
    \coordinate (Q8) at (-0.46,-0.18);

    \path[fill=iconblue!18, draw=black]
      (Q1) -- (Q2) -- (Q3) -- (Q4) -- (Q5) -- (Q6) -- (Q7) -- (Q8) -- cycle;

    \foreach \A/\B in {P1/Q1,P2/Q2,P3/Q3,P4/Q4,P5/Q5,P6/Q6,P7/Q7,P8/Q8}{
      \draw[black] (\A) -- (\B);
    }

    % Highlight a short descent chain inside the tableau.
    \fill[iconblue!35] (-0.72, 0.46) rectangle (-0.44, 0.18);
    \fill[iconblue!55] (-0.44, 0.18) rectangle (-0.16,-0.10);

    \draw[black]
      (P1) -- (P2) -- (P3) -- (P4) -- (P5) -- (P6) -- (P7) -- (P8) -- cycle;

    % Cell structure of the Ferrers diagram.
    \draw[black] (-0.44, 0.46) -- (-0.44,-0.10);
    \draw[black] (-0.16, 0.46) -- (-0.16, 0.18);
    \draw[black] (-0.72, 0.18) -- ( 0.12, 0.18);
    \draw[black] (-0.72,-0.10) -- (-0.16,-0.10);
  \end{scope}
\end{tikzpicture}
...
\tikzsetnextfilename{card-personalMinorProblems}
\begin{tikzpicture}[icon]
  \cardframe
...
\tikzsetnextfilename{card-parkingFunctions}
\begin{tikzpicture}[icon]
  \cardframe

  % Parking sign for parking functions.
  \begin{scope}[xshift=-0.06cm]
    \filldraw[iconblue, draw=black, rounded corners=0.08cm]
      (-0.48,-0.62) rectangle (0.60,0.62);
    \draw[white, line width=1.5mm, line cap=butt]
      (-0.16,-0.38) -- (-0.16,0.38);
    \draw[white, line width=1.5mm]
      (-0.16,0.32) -- (0.2,0.32)
      .. controls (0.42,0.32) and (0.42,-0.05) .. (0.2,-0.05)
      -- (-0.16,-0.05);
  \end{scope}
\end{tikzpicture}


\tikzsetnextfilename{card-permutationPattern}
\begin{tikzpicture}[icon]
  \cardframe
  % 4x4 grid of small dots (background)
  \foreach \i in {0,...,3} {
    \foreach \j in {0,...,3} {
      \node[node-dot] at (-0.75 + 0.5*\i, -0.75 + 0.5*\j) {};
    }
  }
  % Highlight a 231 pattern occurrence (positions 2,3,4 rows 3,4,1)
  \node[node-blue] at (-0.25, 0.25) {};
  \node[node-blue] at (0.25, 0.75) {};
  \node[node-blue] at (0.75, -0.75) {};
  % Connect them to show the pattern
  \draw[iconblue, line width=0.8mm, dashed] (-0.25,0.25) -- (0.25,0.75) -- (0.75,-0.75);
\end{tikzpicture}

\tikzsetnextfilename{card-namedPermutations}
\begin{tikzpicture}[icon]
  \cardframe
  % Three permutation "types" as short dot sequences
  % Top row: up-down pattern
  \foreach \x/\y in {-0.6/-0.1, -0.2/0.3, 0.2/-0.1, 0.6/0.3} {
    \node[node-blue] at (\x, \y + 0.45) {};
  }
  \draw[iconblue, line width=0.6mm] (-0.6,0.35) -- (-0.2,0.75) -- (0.2,0.35) -- (0.6,0.75);
  % Middle row: increasing (Grassmannian, one descent)
  \foreach \x/\y in {-0.6/-0.3, -0.2/-0.1, 0.2/0.1, 0.6/0.3} {
    \node[node-black] at (\x, \y) {};
  }
  \draw[black, line width=0.6mm] (-0.6,-0.3) -- (-0.2,-0.1) -- (0.2,0.1) -- (0.6,0.3);
  % Bottom row: 132-avoiding (decreasing then up)
  \foreach \x/\y in {-0.6/0.2, -0.2/0.0, 0.2/-0.2, 0.6/0.3} {
    \node[node-blue] at (\x, \y - 0.65) {};
  }
  \draw[iconblue, line width=0.6mm] (-0.6,-0.45) -- (-0.2,-0.65) -- (0.2,-0.85) -- (0.6,-0.35);
\end{tikzpicture}

\tikzsetnextfilename{card-notationAffinePermutations}
\begin{tikzpicture}[icon]
  \cardframe
  % Circle representing the affine/cyclic structure
  \def\rad{0.6}
  \draw[black, line width=0.8mm] (0,0) circle (\rad);
  % 8 nodes around the circle
  \foreach \i in {0,...,7} {
    \pgfmathsetmacro{\ang}{90 + \i*45}
    \coordinate (n\i) at (\ang:\rad);
  }
  % Arrows showing a signed/affine permutation action
  \draw[iconblue, ->, >=stealth, line width=0.8mm] (n0) to[bend left=20] (n2);
  \draw[iconblue, ->, >=stealth, line width=0.8mm] (n2) to[bend left=20] (n4);
  \draw[iconblue, ->, >=stealth, line width=0.8mm] (n4) to[bend left=20] (n6);
  \draw[iconblue, ->, >=stealth, line width=0.8mm] (n6) to[bend left=20] (n0);
  % Nodes on top
  \foreach \i in {0,2,4,6} {
    \node[node-blue] at (n\i) {};
  }
  \foreach \i in {1,3,5,7} {
    \node[node-black] at (n\i) {};
  }
\end{tikzpicture}


\tikzsetnextfilename{card-kTheory}
\begin{tikzpicture}[icon]
  \cardframe

  % Grothendieck-group relation: a middle object split into subobject
  % and quotient, drawn as bundle-like blocks.
  \draw[black, thinner, ->, >=stealth] (-0.65,0.35) -- (-0.18,0.35);
  \draw[black, thinner, ->, >=stealth] (0.18,0.35) -- (0.65,0.35);

  \filldraw[white, draw=black, rounded corners=0.08cm]
    (-0.82,0.18) rectangle (-0.55,0.52);
  \filldraw[iconblue, draw=black, rounded corners=0.08cm]
    (-0.16,0.12) rectangle (0.16,0.58);
  \filldraw[white, draw=black, rounded corners=0.08cm]
    (0.55,0.18) rectangle (0.82,0.52);

  \draw[black, thinner] (-0.55,-0.35) -- (-0.15,-0.35);
  \draw[black, thinner] (0.15,-0.35) -- (0.55,-0.35);
  \draw[black, thinner] (0,-0.55) -- (0,-0.15);

  \begin{scope}[yshift=-0.35cm]
    \filldraw[white, draw=black, rounded corners=0.07cm]
      (-0.72,-0.18) rectangle (-0.38,0.18);
    \filldraw[iconblue, draw=black, rounded corners=0.07cm]
      (-0.17,-0.18) rectangle (0.17,0.18);
    \filldraw[white, draw=black, rounded corners=0.07cm]
      (0.38,-0.18) rectangle (0.72,0.18);
  \end{scope}
\end{tikzpicture}


\tikzsetnextfilename{card-varieties}
\begin{tikzpicture}[icon]
  \cardframe

  % A clean flag-variety cue.
  \draw[black, line width=0.85mm] (-0.48,-0.62) -- (-0.48,0.62);
  \filldraw[iconblue, draw=black, line width=0.55mm]
    (-0.48,0.52) -- (0.42,0.34) -- (-0.48,0.14) -- cycle;
  \filldraw[white, draw=black, line width=0.55mm]
    (-0.48,0.08) -- (0.26,-0.08) -- (-0.48,-0.24) -- cycle;
  \draw[black, line width=0.75mm] (-0.65,-0.62) -- (-0.28,-0.62);
\end{tikzpicture}


\tikzsetnextfilename{card-transferMatrixMethod}
\begin{tikzpicture}[icon]
  \cardframe

  % Four states with allowed transitions.
  \foreach \x/\y in {-0.55/0.42, 0.55/0.42, -0.55/-0.42, 0.55/-0.42} {
    \filldraw[iconblue, draw=black, line width=0.55mm, rounded corners=0.04cm]
      (\x-0.22,\y-0.22) rectangle (\x+0.22,\y+0.22);
    \node[circle, fill=white, draw=black, line width=0.45mm,
      minimum size=4pt, inner sep=0pt] at (\x,\y) {};
  }

  \draw[black, thinner] (-0.32,0.42) -- (0.32,0.42);
  \draw[black, thinner] (-0.32,-0.42) -- (0.32,-0.42);
  \draw[black, thinner] (-0.55,0.2) -- (-0.55,-0.2);
  \draw[black, thinner] (0.55,0.2) -- (0.55,-0.2);
  \draw[black, thinner] (-0.38,0.25) -- (0.38,-0.25);
  \draw[black, thinner] (0.38,0.25) -- (-0.38,-0.25);
\end{tikzpicture}


\tikzsetnextfilename{card-symmetricFunctionsSpace}
\begin{tikzpicture}[icon]
  \cardframe

  % The standard involution is the most recognizable operator here.
  \node[text=iconblue, font=\boldmath\fontsize{74}{74}\selectfont] at (0,0.02)
    {$\omega$};
\end{tikzpicture}


\tikzsetnextfilename{card-interlacingPolynomials}
\begin{tikzpicture}[icon]
  \cardframe

  % Two interlacing real-root sets on one axis.
  \draw[black, thinner, ->, >=stealth] (-0.75,0) -- (0.78,0);
  \foreach \x in {-0.55,0,0.55} {
    \node[node-blue] at (\x,0) {};
    \draw[iconblue, thinner] (\x,0.16) -- (\x,0.36);
  }
  \foreach \x in {-0.28,0.28} {
    \node[node-black] at (\x,0) {};
    \draw[black, thinner] (\x,-0.16) -- (\x,-0.36);
  }
\end{tikzpicture}


\tikzsetnextfilename{card-stablePolynomial}
\begin{tikzpicture}[icon]
  \cardframe

  % Upper half-plane picture for stability.
  \fill[iconblue!15] (-0.75,-0.25) rectangle (0.75,0.75);
  \draw[black, thinner, ->, >=stealth] (-0.78,-0.25) -- (0.8,-0.25);
  \draw[black, thinner, ->, >=stealth] (-0.42,-0.72) -- (-0.42,0.78);
  \node[node-blue] at (-0.06,0.12) {};
  \node[node-blue] at (0.32,0.32) {};
  \node[node-blue] at (0.58,0.02) {};
\end{tikzpicture}


\tikzsetnextfilename{card-lorentzianPolynomial}
\begin{tikzpicture}[icon]
  \cardframe

  % Lorentz-cone silhouette.
  \fill[iconblue!18] (0,0) -- (-0.72,0.72) -- (0.72,0.72) -- cycle;
  \draw[iconblue, line width=0.9mm] (0,0) -- (-0.72,0.72);
  \draw[iconblue, line width=0.9mm] (0,0) -- (0.72,0.72);
  \draw[black!55, thinner] (0,0) -- (-0.72,-0.72);
  \draw[black!55, thinner] (0,0) -- (0.72,-0.72);
  \node[node-black] at (0,0) {};
\end{tikzpicture}


\tikzsetnextfilename{card-gammaPositivity}
\begin{tikzpicture}[icon]
  \cardframe

  % Symmetric unimodal coefficients with the central gamma contribution.
  \draw[black, line width=0.45mm] (-0.75,-0.6) -- (0.75,-0.6);
  \filldraw[fill=white, draw=black, line width=0.5mm]
    (-0.67,-0.6) rectangle (-0.45,-0.32);
  \filldraw[fill=white, draw=black, line width=0.5mm]
    (-0.39,-0.6) rectangle (-0.17,0.12);
  \filldraw[fill=iconblue, draw=black, line width=0.5mm]
    (-0.11,-0.6) rectangle (0.11,0.35);
  \filldraw[fill=white, draw=black, line width=0.5mm]
    (0.17,-0.6) rectangle (0.39,0.12);
  \filldraw[fill=white, draw=black, line width=0.5mm]
    (0.45,-0.6) rectangle (0.67,-0.32);
\end{tikzpicture}


\tikzsetnextfilename{card-realRootedCatalan}
\begin{tikzpicture}[icon]
  \cardframe

  % A small Dyck path with its peaks marked.
  \draw[black!55, line width=0.45mm] (-0.78,-0.58) -- (0.78,-0.58);
  \fill[iconblue!15]
    (-0.78,-0.58) -- (-0.52,-0.32) -- (-0.26,-0.06)
    -- (0,-0.32) -- (0.26,-0.58) -- (0.52,-0.32)
    -- (0.78,-0.58) -- cycle;
  \draw[iconblue, line width=0.9mm]
    (-0.78,-0.58) -- (-0.52,-0.32) -- (-0.26,-0.06)
    -- (0,-0.32) -- (0.26,-0.58) -- (0.52,-0.32)
    -- (0.78,-0.58);
  \node[node-black] at (-0.26,-0.06) {};
  \node[node-black] at (0.52,-0.32) {};
\end{tikzpicture}


\tikzsetnextfilename{card-realRootedMatching}
\begin{tikzpicture}[icon]
  \cardframe

  % A graph with a highlighted matching.
  \coordinate (a) at (-0.48,0.48);
  \coordinate (b) at (0.48,0.48);
  \coordinate (c) at (-0.48,-0.48);
  \coordinate (d) at (0.48,-0.48);
  \draw[black!55, thinner] (a) -- (b);
  \draw[black!55, thinner] (c) -- (d);
  \draw[black!55, thinner] (a) -- (d);
  \draw[black!55, thinner] (b) -- (c);
  \draw[iconblue, line width=1.1mm] (a) -- (c);
  \draw[iconblue, line width=1.1mm] (b) -- (d);
...
\tikzsetnextfilename{card-newtonPolytopes}
\begin{tikzpicture}[icon]
  \cardframe

  % Newton polygon with its lattice support points.
  \draw[black!45, line width=0.35mm] (-0.62,-0.55) -- (-0.62,0.55);
  \draw[black!45, line width=0.35mm] (-0.62,-0.55) -- (0.65,-0.55);
  \foreach \x in {-0.62,-0.2,0.22,0.64} {
    \draw[black!25, line width=0.25mm] (\x,-0.55) -- (\x,0.55);
  }
  \foreach \y in {-0.55,-0.18,0.19,0.56} {
    \draw[black!25, line width=0.25mm] (-0.62,\y) -- (0.65,\y);
  }
  \filldraw[fill=iconblue!18, draw=iconblue, line width=0.85mm]
    (-0.52,-0.46) -- (0.55,-0.46) -- (0.28,0.42)
    -- (-0.38,0.26) -- cycle;
  \node[node-blue] at (-0.52,-0.46) {};
  \node[node-blue] at (-0.1,-0.46) {};
  \node[node-blue] at (0.32,-0.46) {};
  \node[node-blue] at (0.28,-0.09) {};
  \node[node-blue] at (-0.38,0.26) {};
  \node[node-blue] at (0.28,0.42) {};
\end{tikzpicture}


\tikzsetnextfilename{card-posets}
\begin{tikzpicture}[icon]
  \cardframe

  % A small Hasse diagram with two linear-extension paths suggested.
  \coordinate (a) at (-0.48,-0.48);
  \coordinate (b) at (0.48,-0.48);
  \coordinate (c) at (-0.48,0.12);
  \coordinate (d) at (0.48,0.12);
  \coordinate (e) at (0,0.66);

  \draw[black!55, line width=0.65mm] (a) -- (c);
  \draw[black!55, line width=0.65mm] (b) -- (d);
  \draw[black!55, line width=0.65mm] (c) -- (e);
  \draw[black!55, line width=0.65mm] (d) -- (e);
  \draw[iconblue, line width=0.75mm] (a) -- (d);
  \draw[iconblue, line width=0.75mm] (b) -- (c);
