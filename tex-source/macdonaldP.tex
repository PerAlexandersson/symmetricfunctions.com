...
\begin{figure}
\begin{ytableau}
*(pink) & \ast & *(pink) & *(pink) & *(pink) & \ast & *(gray) & *(gray)\\
*(pink) & *(pink) & *(pink) & *(pink) & *(pink) \\
*(pink) & \ast &  *(gray) & *(gray)& *(gray) \\
*(gray)
\end{ytableau}
\end{figure}
We then have
...
Now, if $T$ is a semi-standard Young tableau,
we let 
\[
...
In \cite{Haglund2010}, Jim Haglund conjectures that 
\[
...
Arun Ram suggests that Haglund's conjecture can be made stronger: there is some type of positivity when $\macdonaldJ_\lambda(\xvec;q,q^k)$
is expanded in the $\macdonaldJ_\lambda(\xvec;q,q^{k-1})$-basis. Note that $\macdonaldJ_\lambda(\xvec;q,q)=\schurS_\lambda$.

Haglund's conjecture has been shown to be true in some special cases,
see \cite{Yoo2012,Yoo2015,Bhattacharya2022x}.
...
A quasisymmetric refinement of $\macdonaldJ_\lambda(\xvec;q,t)$
...
In \cite{GonzalesLapointe2019}, C. González and L. Lapointe introduce the multi-Macdonald polynomials,
...
were conjectured to exist in M. Haiman's seminal article \cite[7.2.19]{Haiman2002hilbertSchemes}.
