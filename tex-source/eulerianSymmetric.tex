...
\begin{polydata}{eulerianSymmetric}
  ...
  Category & Other \\
  PositiveIn & gessel | ShareshianWachs2010 \\
  PositiveIn & schurS | HendersonWachs2012 \\
  PositiveIn & powerSum | SaganShareshianWachs2011 | scope=lambda=(n) \\
\end{polydata}



The Eulerian quasisymmetric functions are in fact symmetric.  They refine the
\hyperref[eulerianPolynomial]{classical Eulerian polynomials} by retaining
cycle-type and descent-set data in addition to the excedance number.
...
For $\sigma \in \symS_n$, let $\overline{\sigma}$ be defined as the word
...
that is, it is the \hyperref[frobeniusCharacteristic]{Frobenius characteristic} of a
...
The stronger statement, positivity in the \hyperref[completeH]{complete homogeneous basis}, is not true.
For example,
\[
...
and exactly $j$ excedances.
In \cite{SaganShareshianWachs2011}, it is shown that
\[
\left(\symS_{\lambda,j}, C_n, \sum_{\sigma \in \symS_{\lambda,j}} q^{\maj(\sigma) - \exc(\sigma)} \right)
\]
exhibits the \hyperref[cyclicSieving]{cyclic sieving phenomenon}, where $C_n$ acts by conjugation.
...
that completes this to a cyclic sieving phenomenon.
\end{conjecture}
...
and thus allows for a definition of \emph{colored Eulerian quasisymmetric functions}.
...
In \cite{Lin2013}, the author shows that these are symmetric functions.
These are believed to satisfy a cyclic sieving phenomenon as above.
\name{Vassilis-Dionyssis Moustakas} studies specializations of colored
quasisymmetric functions and obtains Euler--Mahonian identities for colored
permutations \cite{Moustakas2021}.  The paper uses colored fundamental
quasisymmetric functions as the main basis.


\begin{question}
Can the above be generalized to \hyperref[schurWreath]{wreath product symmetric functions}?
\end{question}
