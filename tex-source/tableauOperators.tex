...
\metakeywords{Bender--Knuth involution, evacuation, promotion, jeu de taquin,
Young tableau operation, tableau crystal operator}


\section[tableauOperations]{Operations on Young tableaux}


We collect several standard operations on tableaux.
...
\begin{itemize}
\item $\revCompl : \symS_n \to \symS_n$ via $\pi \to n+1 - \rev(\pi)$,
  the \defin{reverse-complement operator}\label{reverseComplementOperator}.
\item $\revCompl_m : [m]^n \to [m]^n$ is defined similarly as
$w \to m+1 - \rev(w)$.
\item $BK_i$ is the Bender--Knuth involution.
\item $\evac$ and $\evac^*$ are evacuation and dual evacuation, respectively.
...
We let $\revCompl$ act on skew standard Young tableaux with $n$ boxes by
rotating the shape $180^\circ$
and sending entry $k$ to $n+1-k$.


\subsection[benderKnuth]{Bender--Knuth involutions}

The \defin{Bender--Knuth involution}\label{benderKnuthInvolution}
$BK_i$ is a map on $\SSYT(\lambda/\mu, w)$
...
The resulting filling is again a semistandard Young tableau.
...
Note, however, that the $BK_i$ \emph{do not} satisfy the
\hyperref[braidRelations]{braid relations}!
See the \hyperref[crystalOperators]{Lascoux--Schützenberger involutions}
instead for such a family of involutions.

\name{Álvaro Gutiérrez} gives type $B$ and type $C$ analogues of the
Bender--Knuth involutions \cite{Gutierrez2024}.  These give a combinatorial
proof that the King and Sundaram tableau formulas for the symmetric
polynomials of types $B$ and $C$ are invariant under the corresponding Weyl
groups.




\subsection[jeuDeTaquin]{Jeu de taquin}

A standard method converts a skew SSYT to a straight shape via a series of
slides, called
\defin{jeu de taquin}\label{jeuDeTaquinOperation}.
...
It turns out that the result is independent of the order of the slides,
...
\begin{example*}[Row insertion as jeu-de-taquin]
For the word $w=213$, ordinary row insertion gives
\[
\emptyset \leftarrow 2 = \ytableaushort{2},
\qquad
\ytableaushort{2}\leftarrow 1 = \ytableaushort{1,2},
\qquad
\ytableaushort{1,2}\leftarrow 3 = \ytableaushort{13,2}.
\]
The same tableau is obtained by placing the letters of $w$ on the
anti-diagonal skew shape from the theorem above and rectifying:
\[
\ytableaushort{{\none}{\none}3,{\none}1,2}
\xrightarrow{\jdt}
\ytableaushort{13,2}.
...
performing jeu-de-taquin gives the same result as row insertion of the
reading word of $T$.

Recall the notion of \hyperref[tableauDescentSet]{descent} for skew SYT.
\name{William F. Doran IV} proves that jeu-de-taquin preserves the descent
set \cite[Lem.~3.2]{DoranIV1997}.
This can be generalized to semistandard Young tableaux.

\name{Nohra Hage} studies a super version of jeu de taquin, related to a
Littlewood--Richardson rule for the \hyperref[hookSchur]{super Schur functions}
\cite{Hage2022}.
...
We define \defin{promotion}\label{tableauPromotion}, $\promotion$, on a
...
\name{Marcel-Paul Schützenberger} showed that $\promotion^n(T)=T$
for any standard Young tableau $T$ of \emph{rectangular} shape, with $n$ boxes.
For SYT of shape $2 \times k$, a bijection to non-crossing matchings sends
promotion to rotation; see
\hyperref[cspNonCrossingMatchingsI]{this example of cyclic sieving}.
...
whose promotion orbit has order $n$, are equinumerous with $\symS_n$.


Promotion can be defined on \hyperref[linearExtension]{linear extensions} of
posets, see \name{Richard Stanley} \cite{Stanley2009}.  \name{Colin Defant}
and \name{Noah Kravitz} generalize this to arbitrary
\hyperref[labeledPoset]{poset labelings} through
\defin{promotion sorting}\label{promotionSorting}
\cite{DefantKravitz2022}.
\name{Colin Defant} introduces
\defin{toric promotion}\label{toricPromotion}, a cyclic analogue of
promotion for graph labelings \cite{Defant2022ToricPromotion}.  For forests,
the orbit structure has a particularly simple description.
...
\cite{Schutzenberger1963,Schutzenberger1972}.
We follow \cite{BennettMadillStokke2014}. Note that we define the inverse here,
as this is consistent with previous notation.

The \defin{$k$-promotion operator}\label{kPromotionOperator}
$\promotion_k$ may be defined via jeu-de-taquin as follows.
...
Subtract $1$ from all entries in the tableau and finally fill the cells of
$\lambda/\mu$ with $k$. The result is a semistandard tableau of shape $\lambda$.
...
The \defin{evacuation}\label{tableauEvacuation} operator was introduced by
...
It is a special case of the Lusztig involution on crystals, see e.g.
\cite{MonicalPechenikScrimshaw2018}.
...
As a second example, taken from \cite{PonWang2011} and adapted to our
notation, $\evac$ sends the
...
\defin{dual evacuation}\label{dualEvacuation}, $\evac^*$,
...
\name{Michael Chmutov}, \name{Gabriel Frieden}, \name{Dongkwan Kim},
\name[Joel B. Lewis]{Joel Brewster Lewis}, and \name{Elena Yudovina}
construct an \defin{affine evacuation}\label{affineEvacuation} operator on tabloids
\cite{ChmutovFriedenKimLewisYudovina2022}.  The fixed points of the affine
evacuation involution are counted by a Green polynomial evaluated at $q=-1$.
...
\begin{problem}[See \cite[Rem.~2.12]{PonWang2011}]
...
study promotion on oscillating and alternating tableaux, relating it to
rotation of matchings and permutations \cite{PfannererRubeyWestbury2020}.

\subsection[RSKvsDualRSK]{RSK vs. dual RSK}

\todo{Find a published reference for the RSK, dual RSK, and evacuation proposition.}
...
\end{proposition}
\begin{proof*}
First, \cite[Prop.~2.3.14]{Butler1994} states that for any word,
\[
...
Further properties of RSK say that
\[
\rev(w) \rskDualArrow P \iff \std(w) \rskArrow \std(P^t),
\]
so by standardization, the proposition holds for all biwords
...
\section[tableauOpRelations]{Relation with crystal operators}

The operators above interact well with the type $A$ crystal operators on words
and tableaux.  The basic principle is that \hyperref[rsk]{RSK} transports
crystal operators on one row of a biword to crystal operators on one of the two
tableaux, while the other tableau is unchanged.  Evacuation reverses the
crystal arrows, exchanging $\cryse_i$ with $\crysf_{n-i}$.


\subsection[crystalsRSK]{Crystals and RSK}

\name{David Rush} uses promotion, evacuation, and cyclic sieving in his study of
plethysm coefficients \cite{Rush2018}.
...
Caveat! In order to apply an operator to row $1$ or $2$ of a biword,
...
We now illustrate how to apply $\cryse_1$ on the top row of a biword.
...
\end{pmatrix}
\]
The biword is then re-sorted according to the top row:
\[
\begin{pmatrix}
1&1&1&2&2&3 \\
1&1&3&1&1&2
...
We also have the following relations with evacuation, see
\cite[Prop.~2.87]{Shimozono2005}.
Here $T$ is a semistandard tableau with $n$ boxes.
\begin{itemize}
...
Note that if $\equiv$ denotes Knuth equivalence, then
\cite[Thm.~5.5.1]{Lothaire2002}
\[
...
\name[Marc A. A. van Leeuwen]{Marc van Leeuwen}'s double-crystal
viewpoint gives a reference for the compatibility statements in this
subsection \cite{VanLeeuwen2006}.  In that paper, horizontal and vertical
crystal operations on binary matrices commute, and the resulting matrix
decomposition is related to the dual RSK correspondence; see especially
\cite[Prop.~3.3.4]{VanLeeuwen2006}.  We use the dual RSK convention from
\hyperref[rskDual]{the RSK page}; van Leeuwen notes that the common variants
differ by transposition and Schützenberger duality.
...
This is the row-word form of applying the corresponding matrix crystal
operator in the horizontal direction.
...
The Burge case is covered by the same double-crystal framework.  In
\cite[Prop.~3.3.2]{VanLeeuwen2006}, the integral matrix decomposition is
identified with the Burge correspondence, so the statement below is the
Burge specialization of the horizontal/vertical crystal compatibility.
...
This version of RSK is studied extensively by \name{Olga Azenhas}, see \cite{Azenhas2006},
in particular in relationship with \hyperref[key]{key polynomials}.


\section[tableauOpOther]{Other operators}

There are also the \hyperref[cyclage]{cyclage} and
\hyperref[catabolism]{catabolism} operations on semistandard Young tableaux.
