...
\metadescription{Background on positroids, including the totally nonnegative Grassmannian, Grassmann necklaces, planar networks, lattice path matroids, and rook matroids.}


\section[positroid]{Positroids}

A \defin{positroid} is a \hyperref[matroid]{matroid} that is represented by
a point in the totally nonnegative Grassmannian. Positroids were introduced by
\name{Alexander Postnikov} in his study of total positivity, Grassmannians and
planar networks \cite{Postnikov2006x}.  For the matrix total-positivity
background used in path models, see
\hyperref[totallyNonnegative]{totally non-negative matrices} and
\hyperref[tnnPathMatrix]{path matrices}.

The cyclic order on the ground set is part of the structure. Thus one usually
...
\]
The nonzero Pl{\"u}cker coordinates then define a matroid:
\[
  \mathcal{B}(A)
  =
...
decorated permutations, Grassmann necklaces, Le-diagrams and reduced plabic
graphs. The Grassmann-necklace description is close to the language of
Schubert matroids.
...
A \defin{Grassmann necklace} of type $(k,n)$ is a sequence
\[
...
\[
  ...
  \right\}.
\]
The condition $I_a\leq_a J$ says that $J$ is a basis in a Schubert matroid
with respect to the cyclic order $\leq_a$. Hence
...
$[n]$, and put nonnegative weights on the edges. A boundary measurement matrix
records weighted sums of paths from boundary sources to boundary sinks. By the
\hyperref[lgvLemma]{Lindstr\"om--Gessel--Viennot lemma}, its maximal minors are
sums of weights of non-intersecting path families, and hence are nonnegative.
...
Every uniform matroid $U_{k,n}$ is a positroid. Choose real numbers
...
Let $I=\{i_1\lt \dotsb \lt i_k\}\subseteq [n]$. The corresponding Schubert
matroid has bases
\[
  \{j_1\lt \dotsb \lt j_k\}
...
\]
Schubert matroids, and their cyclic shifts, are positroids. More generally,
...
Every lattice path matroid is a positroid \cite{Oh2011}. In particular, the
Catalan matroids obtained from Dyck paths are positroids. For the definition
and examples, see \hyperref[latticePathMatroids]{the page on lattice path
matroids}.
\end{example}

\begin{example}[Rook matroids]
Alexandersson and Jal proved that every rook matroid is a positroid
\cite{AlexanderssonJal2024x}. For the construction from non-nesting rook
placements on skew Ferrers boards, see \hyperref[rookMatroids]{the rook
matroids section}.
\end{example}
...
\hyperref[matroidDuality]{matroid duality}, restriction, deletion and
contraction. In particular, every \hyperref[matroidMinors]{minor} of a
...
\hyperref[matroidPolytopes]{matroid base polytope} is
\[
  P_M = \operatorname{conv}\{\mathbf{e}_B : B\in \mathcal{B}(M)\}
  \subseteq \setR^n.
\]
For a cyclic interval $[i,j]$, write
\[
...
Thus positroid polytopes form a distinguished subclass of matroid base
polytopes with far fewer defining rank inequalities than a general matroid
base polytope. Moreover, every face of a positroid polytope is again a
positroid polytope, and the face poset embeds naturally into a poset of
weighted non-crossing partitions \cite{ArdilaRinconWilliams2014}.

The closure of a positroid cell in the ordinary Grassmannian is called a
\defin{positroid variety}. These varieties include Schubert varieties and are
one of the geometric reasons that Schubert matroids and their cyclic shifts
