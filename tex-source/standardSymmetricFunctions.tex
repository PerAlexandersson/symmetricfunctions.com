...
cohomology and quantum cohomology of Grassmannians \cite{Weinfeld2019x}.
...
cylindric reverse plane partitions and prove positivity properties for the
resulting families.

\name{Milo Bechtloff Weising} introduces \defin{Artin symmetric functions}
\cite{Weising2024x}, attached to Galois representations over number fields.
...
\name{Milo Bechtloff Weising} also studies \defin{multi-symmetric Schur
functions} \cite{Weising2025x}.  These are stable limits of key polynomials,
...
- PositiveIn augmentedMonomial -> monomial (current guess: monomial -> monomial); from polynomialData.m, tikzdataPositivity.txt.
}
...
\todo{Legacy relation hints from old polynomialData.m/tikzdata*.txt to verify or migrate:
- Contains nonSymElem -> elementaryE (current guess: elementaryE -> elementaryE); from polynomialData.m, tikzdataRelations.txt.
- PositiveIn nonSymElem -> key (current guess: elementaryE -> key); from polynomialData.m, tikzdataPositivity.txt.
}
...
\todo{Move this to a separate file/doc together with Fock space.}
...
\begin{lemma}
Let $f(t)$ be a formal power series with $f(0)=1$.
Then 
\[
...
This map has an important role for the \hyperref[chromaticQuasisymmetric]{chromatic symmetric functions}, see \cite{Stanley1995}.
In particular, it has the property that $\phi(\elementaryE_mu) = t^{\length(\mu)}$.
...
The algebra of symmetric functions admits a \hyperref[hopfAlgebra]{Hopf algebra structure},
...
The Hopf algebra \defin{antipode} on symmetric functions 
...
is a filling of $\mu$ with rim hooks, such that the length of the rim hooks are given by $\lambda$
and each rim hook has at least one box in
