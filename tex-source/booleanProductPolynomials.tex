...
\begin{polydata}{booleanProduct}
  ...
  Year     & 2018 \\
  Category & Other \\
  PositiveIn & schurS | BilleyRhoadesTewari2019 \\
\end{polydata}


The \defin{boolean product polynomials} are a family of polynomials defined as follows:
\[
\booleanProduct_{k}(\xvec_n) \coloneqq 
\prod_{ 1 \leq i_1 \lt i_2 \lt \dotsb \lt i_k \leq n } (x_{i_1} + x_{i_2}+\dotsb + x_{i_k}).
\]
These were introduced by \name{Lukas Billera}, \name{Sara Billey}, and \name{Vasu Tewari} in an FPSAC abstract,
and inspired by certain applications in hyperplane arrangements, among other things.

\begin{example}
% Related Rust: sym-poly/sym/examples/boolean_product_site_example.rs
The first nontrivial case is already Schur-positive:
\[
\begin{aligned}
\booleanProduct_2(\xvec_3)
&=(x_1+x_2)(x_1+x_3)(x_2+x_3)\\
&=\monomial_{21}+2\monomial_{111}
=\schurS_{21}.
\end{aligned}
\]
\end{example}
...
The authors use a new method for proving \hyperref[schurPositivity]{Schur positivity}, called \emph{Chern plethysm}.
...
\name{Nathaniel Libman} and \name{Gidon Orelowitz} give combinatorial
interpretations for Schur expansions of special cases of Chern plethysm
\cite{LibmanOrelowitz2023x}.  They also construct a symmetric-group module
whose Frobenius characteristic realizes one of these symmetric-function
analogues.
