...
\hyperref[schubertVarietiesAndMatroids]{Schubert varieties} and their intersection
products to solve enumerative problems.  In the most classical case, this means
computing in the cohomology ring of a \hyperref[grassmannianVarieties]{Grassmannian}.
...
$W\subseteq V$; see also the \hyperref[grassmannianVarieties]{varieties page}
for the Plücker-coordinate viewpoint.
...
$|\lambda|$.  These are the Grassmannian instances of the
\hyperref[schubertVarietyGeometryDefinition]{Schubert varieties} described on
the varieties page.
...
For the flag-variety polynomial analogue, compare the
...
\begin{example*}[A dual Schubert class in $\operatorname{Gr}(2,4)$]
...
Schubert calculus on the \hyperref[flagVarieties]{complete flag
...
For the full \hyperref[flagVarieties]{flag variety} in $\setC^3$, with
permutations written in one-line notation, the six
\hyperref[schubertOperatorformula]{divided-difference Schubert polynomials}
are
\[
\begin{array}{c|c}
...
computation; Grassmannian permutations recover the Schur-polynomial subcase,
as explained on the \hyperref[grassmannianPermutation]{Schubert polynomial}
page.
