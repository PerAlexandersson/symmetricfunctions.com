...
described. In fact, it generalizes to \hyperref[jackP]{Jack polynomials}.
\begin{theorem}
...
Groebner basis for the small quantum cohomology ring of the Grassmannian to
...
This is the definition in \hyperref[rootSystem]{root system} type $A_n$ --- there are analogs of 
...
The number of summands used in expressing $w$ converges to a Gaussian distribution,
...
This can perhaps be seen as a type $B$ analog of Steinberg's formula.
...
By using the \hyperref[gtKostka]{Gelfand--Tsetlin polytope model for semistandard Young tableaux},
...
This fact follows from \cite[Thm.~1]{Sturmfels1995} together with the fact that the
...
\name{Anatol Kirillov} provides an extensive survey on Kostka--Foulkes polynomials in \cite{Kirillov2001}.
...
\name[M. P. Schützenberger]{Marcel-Paul Schützenberger} in \cite{LascouxSchutzenberger78}. 

First we need to introduce the \defin{charge}\label{chargeStatistic} of a
permutation.
...
One can show that the above formula generalizes, see \cite[Cor.~2.5.8]{Butler1994}.
\[
...
\]
where $\equiv$ denotes \hyperref[knuthRelations]{Knuth equivalence}. This is in fact a graded poset,
...
We introduce $\kostantP^\eta_q(w)$, which is a generalization of the $q$-analog of the 
