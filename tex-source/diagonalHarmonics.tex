...
ordered set partitions related to the Delta conjecture, see
...
\]
The \hyperref[SnRepresentationTheory]{symmetric group} $\symS_n$ acts
diagonally by permuting the variables in each alphabet, making the space of
diagonal harmonics into a bigraded
\hyperref[SnRepresentationTheory]{$\symS_n$-module} $\mathcal{DH}_n$.
...
Let $f$ be any symmetric function. We define operators 
...
operators interact with Delta operators and plethystic substitutions.
...
Let $DH_n(q,t)$ be the bigraded \hyperref[frobeniusCharacteristic]{Frobenius characteristic} of 
...
Vandermonde determinants indexed by Dyck paths \cite{Jiang2025x}.  The
...
coinvariants indexed by $m$-Dyck paths.
...
components.
\name{Jesse Kim} and \name{Brendon Rhoades} study set partitions, fermions,
...
\name{Mark Haiman} proved in \cite[Thm. 3.10]{Haiman2002} a connection with Macdonald polynomials,
...
that $H_{(m,n)}$ is a positive linear combination of vertical-strip \hyperref[lltDefinitionTuples]{LLT polynomials}.
...
Hikita polynomials and are related to parabolic affine Springer fibers.  Their
finite rational shuffle theorem uses rank-$r$ semistandard parking functions,
...
\name{Viviane Pons} gives an explicit description of the zeta map on Dyck
paths in terms of area sequences \cite{Pons2022x}.  This provides a direct and
implementable way to see how the zeta map sends the pair of statistics
$(\dinv,\area)$ to $(\area,\bounce)$.


\name{Anton Mellit} proved the following theorem in \cite{Mellit2016x}.
...
\hyperref[hallInnerProduct]{Hall inner product}, see \cite[Eq.(4.6) and (6.29)]{BergeronGarsiaLevenXin2015}.
...
from the Rational Shuffle theorem by applying a Schur skewing operator
...
\defin{Hopf chain}: a nested tuple of Dyck paths
...
\name{Emily Sergel} \cite{Sergel2017} gave a proof of the square path conjecture.
...
\todo{
Thm 2.1 in \cite{Sergel2017} is reminiscent of the q-product for acyclic orientations!
...
\name{James Haglund}, \name{Jeffrey Remmel}, \name{Andrew Wilson}
\cite{HaglundRemmelWilson2018}.  It has two versions, the \defin{rise version}
and the \defin{valley version}.  The rise version is now a theorem: it follows
...
two-row Delta Springer varieties, introduced by Griffin--Levinson--Woo to
...
The \defin{Generalized delta square conjecture} (stated in
...
\name{Nicholas Loehr} and \name{Gregory Warrington} \cite{LoehrWarrington2010} conjectured 
...
\name{Menghao Qu} and \name{Guoce Xin} prove Schur positivity of
