...
\]
Moreover, we say that $f$ \defin{alternates left of} $g$ if
$\deg(f)=\deg(g)=d$ and
\[
...
have a \defin{common interlacing} if there is some $h \in \setR[z]$
...
\]
This is the \defin{symmetric $I_d$-decomposition} of $p$, and it is given
explicitly by
\[
...
the linear map $D : \setR[x] \to \setR[x]$ by
\[
...
in $[-1,0]$, write $p \leq_{\mathrm{root}} q$ if
