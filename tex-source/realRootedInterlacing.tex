...
real and non-positive.  If $g \interl f$, then $f-tg$ is real-rooted and
...
\]
have non-negative coefficients.  Form the
...
\end{pmatrix}.
\]
Then $H(p_0,p_1)$ is \hyperref[totallyNonnegative]{totally non-negative}
...
\begin{enumerate}
\item All entries of $G$ have non-negative coefficients, and
...
non-negative coefficient setting, this can be read as an interlacing relation.
...
Assume that both $a$ and $b$ have non-negative coefficients. Then the
...
\]
where $W(f)(z)$ and $W(g)(z)$ are real-rooted with non-positive roots.
Then $W(fg)(z)$ also has real and non-positive roots.
\end{theorem}
