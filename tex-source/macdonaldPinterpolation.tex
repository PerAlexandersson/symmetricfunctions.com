...
The interpolation Macdonald polynomials (in type $A$) are a family of non-homogeneous polynomials
...
symmetric in $x_1t^{n-1},x_2 t^{n-2},\dotsc,x_n$, and satisfies
\[
...
In \cite{Okounkov1998shifted} \name{Andrei Okounkov} proves the following combinatorial formula for
...
and $\psi_{T}(q,t)$ is the \hyperref[macdonaldPTableauFormula]{same weight} which appears in the formula for
the classical Macdonald $P$ polynomials $\macdonaldP(\xvec;q,t)$.

\name{Houcine Ben Dali} and \name{Lauren Williams} give a combinatorial
formula for interpolation Macdonald polynomials in terms of signed multiline
queues \cite{DaliWilliams2025x}.  Their formula generalizes the
multiline-queue formula for ordinary Macdonald polynomials of
