...
polynomial, or into an entire power series, called its \defin{symbol}.  This
is often the most practical way to prove that an operator preserves stability:
instead of testing the operator on all stable inputs, one tests one universal
object.  For real stability one must allow both signs.  For instance,
...
\]
define the two \defin{algebraic symbols} of $T$ by
\[
 G_T^{+}(\xvec,\mathbf{w})
 \coloneqq
...
Let \(\mathcal{LP}_r\) be the class of entire functions in \(r\) variables
...
\]
its \defin{transcendental symbol} is the formal power series
\[
...
\begin{example*}[Multivariate Eulerian polynomials]\label{ex:stableMultivariateEulerianOperator}
