\metatitle{Loop Schur polynomials}
\metadescription{An introduction to loop Schur polynomials, their definition via tableaux, Jacobi--Trudi identities, alternant quotient identity, and Murnaghan--Nakayama rule.}
\metakeywords{Loop Schur polynomials,loop symmetric functions,murnaghan-nakayama,fillings,jacobi-trudi}
...
\begin{polydata}{schurLoop}
  ...
  Category & Schur \\
  SpecializesTo & schurS | LamPylyavskyy2012 | map=r=1 \\
\end{polydata}
...
x_{i_1}^{(r)} x_{i_2}^{(r+1)} \dotsb x_{i_k}^{(r+k-1)}
\]
\[
\completeH_{k}^{(r)}(\xvec) \coloneqq \sum_{1\leq i_1 \leq i_2 \leq \dotsc \leq i_k \leq m}
x_{i_1}^{(r)} x_{i_2}^{(r-1)} \dotsb x_{i_k}^{(r-k+1)}
\]
where the upper indices are taken modulo $n$.
...
\name{Shinsuke Iwao}, \name{Kohei Motegi}, and \name{Ryo Ohkawa} obtain
tensor Schur polynomials and $q$-deformed loop elementary symmetric functions
as three-dimensional partition functions of tetrahedral $L$-operators
\cite{IwaoMotegiOhkawa2026x}.  At $q=0$ their construction gives products of
Schur polynomials and recovers the Schur shuffle formula; at generic $q$ it
produces explicit deformations of the loop elementary functions.
...
\[
\mathrm{wt}_r(T) \coloneqq
\prod_{(i,j)\in \lambda/\mu} x_{T(i,j)}^{(r+c(i,j))}.
\]
The \defin{loop Schur polynomial} is then defined as
\[
\schurS^{(r)}_{\lambda/\mu}(x_1,\dotsc,x_m) \coloneqq 
\sum_{T \in \SSYT(\lambda/\mu)} \mathrm{wt}_r(T).
\]
...
\end{ytableau}
\end{figure}
where the superscripts indicate the colors.
\end{example}
...
The idea of the proof is attributed to \name{Greg Anderson}.
...
\schurS^{(r)}_{\mu}(x_1,\dotsc,x_m)
\]
where the sum is over all partitions $\mu$ with at most $m$
parts, such that $\mu/\lambda$ is a ribbon of size $kn$.
\end{theorem}
For notation and terminology, see 
\hyperref[schurMurnaghanNakayama]{the Murnaghan--Nakayama rule for Schur functions}.
