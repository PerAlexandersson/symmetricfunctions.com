...
\metadescription{Introduction to lattice path matroids, including their
relation to transversal matroids, Tutte polynomials, skew Ferrers shapes, Dyck
paths, and width-two posets.}

\section[latticePathMatroids]{Lattice path matroids}


The family of \defin{lattice path matroids}\label{latticePathMatroidDefinition}
was introduced by \name{Joseph Bonin}, \name[A. de Mier]{Anna de Mier}, and
\name{Marc Noy} \cite{BoninMierNoy2003}.  This family is closed under all
\hyperref[matroidMinors]{matroid minors} (deletions and contractions) and also
under \hyperref[matroidDuality]{matroid duality}.
All lattice path matroids are \hyperref[transversalMatroids]{transversal matroids}.
They are also \hyperref[positroid]{positroids}, see \cite{Oh2011}.
...
$(a,b)$-Catalan \hyperref[matroidBasePolytope]{matroid polytope} as a
weighted sum of indicator functions of direct sums of uniform matroids.  This
gives explicit formulas for valuative invariants, including Ehrhart positivity
and Kazhdan--Lusztig invariants for these Catalan matroids.
...
the \hyperref[tutteInternalExternal]{internal and external activity} for the
bases.
...
For a broader introduction to transversal matroids, see the lecture notes by
\name{Joseph Bonin} \cite{Bonin2010TransversalNotes}.
...
\)
Numbering the skew boxes by the elements of $A_5,\dotsc,A_1$ gives
\begin{figure}
\svgimg[width=0.2\textwidth]{./svg-images/latticepath-matroid2.svg}{The skew Ferrers shape and labeling of the North steps.}
\end{figure}


The upper boundary has North-step set $\{1,3,5,6,10\}$ and East-step set
$\{2,4,7,8,9,11,12\}$.

The corresponding naturally labeled \hyperref[posetWidth]{width-two}
permutation poset has chains
\[
  C_N = \{1 \lt 3 \lt 5 \lt 6 \lt 10\},\qquad
  C_E = \{2 \lt 4 \lt 7 \lt 8 \lt 9 \lt 11 \lt 12\},
\]
together with the cover relations $2\lt 3$, $4 \lt 5$, $9 \lt 10$, and
$1 \lt 12$.
\begin{figure}
 \svgimg{./svg-images/latticepath-width-two-poset.svg}{The width-two poset associated with the lattice path matroid example.}
\end{figure}
The permutation $\pi = [2,4,7,8,9,11,1,12,3,5,6,10]$ defines a permutation
poset, and all \hyperref[linearExtension]{linear extensions} of the
width-two poset above are precisely the permutations below $\pi$ in the
\hyperref[weakOrder]{weak order}. Note that $\pi$ is 321-avoiding.
...
computes the number of \hyperref[linearExtension]{linear extensions} of the
associated width-two permutation poset.


We can also model set systems using a Dyck path.
Each set in the set system corresponds to a peak.
The peaks are marked with the corresponding set and the shaded boxes are those
below and to the right of each peak.
\begin{figure}
\begin{ytableau}
...
diagonal, $d_1,d_2,\dotsc,d_k$, such that peak $i$ \enquote{can see} the square
$d_i$.  Here, $k$ is the number of peaks.


In this model, the rank of the lattice path matroid is the number of peaks of
the Dyck path, and the dimension of its base polytope is the number of nonzero
entries in the area sequence.
Area sequences of the form $[0,1,2,\dotsc,k,k,\dotsc,k]$ correspond to
\hyperref[uniformMatroid]{uniform matroids}.
A combinatorial formula for the $h^*$-vector is provided in \cite{Kim2020};
see also \cite{Li2012}.


%If the Dyck path contains $N^2 E N^2 E^2 N E^2$ as a subsequence, then we have Q6.
\end{example}

The corresponding determinant formula for the lattice-path-matroid basis
generating function is given in \hyperref[LGVLatticePathsInSkewShape]{the LGV
section on lattice paths in skew shapes}.


\subsection[snakeMatroidShardPolytopes]{Snake matroids and shard polytopes}

The moment polytope of the
\hyperref[qsymToricRichardsonGeometry]{toric Richardson variety} associated
with a binary word is the base polytope of the corresponding snake matroid;
these are precisely the shard polytopes.  Its $h$-vector counts distinct
subwords by length, and the $f$-vector of every snake-matroid base polytope is
log-concave \cite[Thm.~V and Cor.~VI]{GonzalesLeLowen2026x}.
...
\defin{non-attacking}\label{nonAttackingRookPlacement} if no two rooks share a
row or a column. Two rooks form a \defin{nesting}\label{rookNesting} if one
lies strictly southeast of the other. A \defin{non-nesting rook placement}
\label{nonNestingRookPlacement} is a non-attacking placement with no nesting.
Let
\[
  \mathrm{NN}_{\lambda/\mu}
\]
denote the set of non-nesting rook placements on $\lambda/\mu$.
The corresponding \defin{non-nesting rook polynomial}
\label{nonNestingRookPolynomial} is
\[
  M_{\lambda/\mu}(t) \coloneqq \sum_k r_k(\lambda/\mu)t^k,
\]
...
The \defin{rook matroid}\label{rookMatroidDefinition} of $\lambda/\mu$ has
bases
\[
...
\begin{theorem}[Alexandersson--Jal, \cite{AlexanderssonJal2024x}]
Rook matroids are closed under direct sums and under
\hyperref[matroidDuality]{matroid duality}.  More precisely,
\[
...
polynomials agree and that the \hyperref[ehrhart]{Ehrhart polynomials} of the
two matroid base polytopes agree, even when the matroids are not isomorphic.

The base polytopes of lattice path matroids have positive Ehrhart coefficients,
see \cite{FerroniMoralesPanova2026x}.

\name{Douglas M. Chen}, \name{Mario Sanchez}, \name{John Veliz}, and
\name{Zhiyan Ying} introduce lattice path delta-matroids, a delta-matroid
analogue of the lattice path construction \cite{ChenSanchezVelizYing2023x}.
...
\cite{BrandenHuh2020,AlexanderssonJal2024x}.
\hyperref[realRootedPolynomial]{Real-rootedness} can still fail.
For the square-case shapes attached to generalized snake posets, however,
\name{Benjamin Braun} and \name{Aryaman Jal} prove real-rootedness and an
\hyperref[interlacingPolynomials]{interlacing} recurrence
\cite{BraunJal2026x}; equivalently, these order
polytopes have real-rooted \hyperref[hStarPolynomial]{$h^*$-polynomials}.
...
\]
where $W_P(t)$ is the
\hyperref[pEulerianPolynomial]{$P$-Eulerian polynomial}. This refines the
width-two poset example above and is compatible with the
\hyperref[weakOrder]{weak order} description for ordinary Ferrers shapes.
