...
and are characters for the Lie group $Sp(2n)$. See also \cite{Krattenthaler1998} for a reference.
...
\hyperref[planePartitions]{shifted plane partitions} of shifted double staircase shape
...
\hyperref[flagVarieties]{flag varieties} and gives defining equations for these varieties
\cite{Balla2022}.
...
\hyperref[kashiwaraCrystals]{$U_q(\mathfrak{sp}(2m,\setC))$-crystal structure}; see
\cite{AzenhasFellerTorres2022}.
...
The \emph{odd symplectic Schur functions} have a more complicated bialternant formula,
see \cite{Okada2020OddSymplectic}.
...
In \cite{Lee2019}, \name{Seung Jin Lee} defines a \hyperref[crystalGraphs]{crystal graph} structure on King tableaux,
...
functions obtained by adding a \hyperref[skewShapes]{border strip} to $\mu$, those obtained by
removing a \hyperref[skewShapes]{border strip} from $\mu$, and an additional Weyl-denominator
...
There are analogues of \hyperref[gtpatterns]{Gelfand--Tsetlin patterns} for these functions, see e.g.
\cite{Proctor1994,AyyerFischer2019,JingLiWang2024}.

\name{Tewodros Amdeberhan}, \name{George E. Andrews}, and
\name{Cristina Ballantine} study partition identities involving \hyperref[partitionCores]{hook lengths}
...
The even symplectic Schur functions can be expressed as a sum of skew Schur functions as
\[
\schurSp_\lambda(\xvec) = \sum_\alpha (-1)^{|\alpha|/2} \schurS_{\lambda/\alpha}(\xvec)
\]
where $\alpha$ ranges over partitions whose \hyperref[prelimPartitions]{Frobenius coordinates} are
...
We then define the \defin{even orthogonal Schur polynomials}
\label{evenOrthogonalSchurPolynomial} (using the definition in \cite{Hamel1997}) as
\[
...
We note that $\omega(\schurSp_\lambda)=\schurOr_{\lambda'}$, where $\omega$ is the
\hyperref[standardInvolution]{standard involution on symmetric functions}.
...
\defin{Schur's $P$ function}\label{schurPFunction} can then be defined as
\[
...
An alternative combinatorial model uses semistandard decomposition tableaux, introduced in \cite{Serrano2010}.
...
\hyperref[schurPositivity]{Schur positivity} using \hyperref[crystals]{crystals}, see \cite{ChoiKwon2018}.
...
The Schur $Q$ functions are related to so-called
...
\]
Rosengren mentions that this formula is in fact equivalent to \hyperref[schurKawanaka]{Kawanaka's identity} for
Schur functions.
...
A Hall--Littlewood analogue of $\schurQ_\lambda(\xvec)$ is introduced in \cite{TudoseZabrocki2003}.
...
\hyperref[plethysm]{Plethysm} stability for Schur's $Q$-functions is considered in \cite{GrafJing2024x}.
...
Let the degree of the \hyperref[powerSum]{power-sum symmetric functions} be $1$, so that $deg(\powerSum_i)=1$
and $deg(\powerSum_\nu) = \length(\nu)$. The
...
\]
The \defin{bottom Schur function}\label{bottomSchurFunction}
$\hat{\schurS}_\lambda$ is defined as
\[
...
In 2019, an OEIS search suggested that the sequence $a(n)$ given by \oeis{A129528}
is a match.
The sequence $a(n)$ is defined as follows:
Consider the set of all Dyck paths, consisting of steps $(1,1)$ and
$(1,-1)$.
The \defin{peak-abscissa-sum}\label{peakAbscissaSum} of a path is the sum of
the $x$-coordinates of all peaks.
Then $a(n)$ is the number of Dyck paths of \emph{any length} with
peak-abscissa-sum $n$.
...
\name{Sylvie Corteel} and \name[J. S. Kim]{Jang Soo Kim} introduce lecture hall
Schur functions in \cite{CorteelKim2018}.  They prove analogues of the
Jacobi--Trudi identity and its dual.
...
A \defin{lecture hall tableau}\label{lectureHallTableau} of type $(n,\geq,>)$
and shape $\lambda/\mu$
...
For asymptotics of lecture hall tableaux, see \cite{CorteelKeatingNicoletti2019x}.
...
Wreath product Schur polynomials appear in the study of $G \wr \symS_n$;
see, for example, \cite{Macdonald1980} and \cite{MendesRemmelWagner2004}.
...
We then define the \defin{wreath product Schur polynomials}
\label{wreathProductSchurPolynomial} as
\[
\schurS_{\underline{\lambda}}(\xvec^{(1)},\dotsc,\xvec^{(r)}) = \prod_{i} \schurS_{\lambda^i}(\xvec^{(i)}).
\]
This can be realized as a sum over $r$-tuples of semistandard Young tableaux of shape $\lambda^i$, where tableau $i$ only uses
