...
\metadescription{Shifted Jack polynomials, their vanishing definition, reverse
semistandard tableaux formula, and Littlewood--Richardson coefficients.}
\metakeywords{shifted Jack polynomials,shifted symmetric functions,reverse
semistandard tableaux,Littlewood--Richardson coefficients}
...
\begin{polydata}{jackShifted}
  ...
  Category & Schur \\
  SpecializesTo & jackP | OkounkovOlshanski1997 | map=top-degree component \\
  SpecializesTo & schurShifted | Sahi1994 \\
\end{polydata}
...
The shifted Jack polynomials constitute a basis for the ring of
\defin{shifted symmetric functions}\label{shiftedSymmetricFunctionDefinition}
$\Lambda^{a}$.
This is the ring of polynomials symmetric in variables $x_i - i/a$.
...
\[
H_\lambda = \prod_{s \in \lambda} (a \cdot \arm_\lambda(s) + \leg_\lambda(s) + 1),
\quad
H'_\lambda = \prod_{s \in \lambda} (a \cdot \arm_\lambda(s) + \leg_\lambda(s) + a).
\]
These are denoted $c_\lambda(a)$ and $c'_\lambda(a)$ in
\cite[Eq.~(10.22)]{Macdonald1995}.


The polynomials $\jackShifted_\mu$ lie in $\Lambda^{a}(x_1,\dotsc,x_n)$, and it
was shown by \name{Friedrich Knop} and \name{Siddhartha Sahi}
\cite{KnopSahi1996} that $\jackShifted_\mu$
...
\end{cases}
\end{equation*}
(Note that the inequality is in the wrong direction in
\cite[p.~70]{OkounkovOlshanski1997}.)

We also define the shifted version of $\jackJ_\mu(\xvec;a)$ as
\[
\jackShiftedJ_\mu(\xvec,a)
\coloneqq H_\mu \jackShifted_\mu(\xvec,a).
\]
...
\subsection[jackShiftedRSSYTDef]{RSSYT formula}

Just as the Jack polynomials can be \hyperref[jackTableauFormula]{defined}
as a sum over \hyperref[reverseSemistandardYoungTableau]{reverse semistandard
Young tableaux}, there is an analogous formula for the shifted Jack
polynomials; see \cite{OkounkovOlshanski1997}.
\begin{equation*}
\jackShifted_\mu(\xvec;a)
= \sum_{T \in \mathrm{RSSYT}(\mu)}
\psi_T(a) \prod_{s \in \mu} (x_{T(s)} - \arm'(s) + \leg'(s)/a),
\end{equation*}
where $\arm'(s) = j-1$ and $\leg'(s) = i-1$ for the box $s=(i,j)$.
Comparing this with the \hyperref[jackTableauFormula]{similar-looking formula}
for the \hyperref[jackP]{Jack P polynomials}, we see that the top degree
component of $\jackShifted_\mu(\xvec;a)$ is given by the ordinary Jack
function $\jackP_\mu(\xvec;a)$.
...
For $a = 1$, $c^{\lambda}_{\mu\nu}(1)$ coincides with the usual
\hyperref[littlewoodRichardsonCoefficient]{Littlewood--Richardson coefficient}.

From the vanishing result by \name{Friedrich Knop} and \name{Siddhartha Sahi}
\cite{KnopSahi1996} and a simple contradiction argument, see
\cite[p.~4434]{MolevSagan1999} for the case $\alpha=1$, it follows that
$c^{\lambda}_{\mu\nu}(a)$ is identically zero unless
$\lambda \supseteq \mu$ and $\lambda \supseteq \nu$.
...
\end{equation*}
where $\psi'_{\lambda/\mu}(\alpha) \coloneqq \psi_{\lambda'/\mu'}(1/\alpha)$.
...
We have computed these for all $\mu,\nu$ with size at most $6$, and the
\filelink{files/jackStructureConstants6.m}{data is available for download
here (1.4 MB)}.

For example, in the data one can find that
$\mathtt{JackStructureConstant}_{311,222,42211}(a)$ is equal to
\begin{align}
