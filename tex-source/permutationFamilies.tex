...
\item \defin{Up-down}\label{permUpDown} --- in one-line notation,
$\pi_1 \lt \pi_2 \gt \pi_3 \lt \pi_4 \dotsb$.
...
\item \defin{Baxter} --- avoids $2-14-3$ and $3-14-2$, see \cite{DulucqGuibert1998}.
...
There are $2^n-n$ Grassmannian permutations in $\symS_n$, see \oeis{A000325}.
See \hyperref[grassmannianPermutation]{the Schubert polynomial page} for more details.

\item A permutation $\pi$ is \defin{bi-Grassmann} if both $\pi$ and $\pi^{-1}$
are Grassmann.  This is equivalent to avoiding 2413.
...
\item \defin{Parity-alternating permutations} are permutations which send odd
numbers to odd numbers and even numbers to even numbers,
see \cite{KebedeRakotondrajao2021}.
...
\[
\begin{pmatrix}
1 & 2 &       \dotsc & k_1   & k_1+1 & k_1+2 & \dotsc & k_1 + k_2 & k_1+k_2+1 & \dotsc \\
k_1 & k_1-1 & \dotsc,   1		 & k_1+k_2 & k_1+k_2-1 & \dotsc & k_1+1 & k_1+k_2+k_3 & \dotsc \\
\end{pmatrix}
\]
Equivalently, we take the unique longest permutation in each of the sets
$\symS_{k_1},\dotsc,\symS_{k_m}$ and produce their
\hyperref[directSumPermutations]{direct sum}.
