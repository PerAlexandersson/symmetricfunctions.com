...
in both cases.
\begin{figure}
<img src="./svg-images/wordCrystal.svg" alt="Crystal on words" style="width:30%; min-width:10em; margin-right:15px;margin-left:15px;"/>
<img src="./svg-images/ssytCrystal.svg" alt="Crystal on SSYT" style="width:30%; min-width:10em; margin-right:15px;margin-left:15px;"/>
\end{figure}
\end{example*}
...
\end{enumerate}
The inner product used here is from the \hyperref[root-systems]{root system}.
...
Otherwise, 
$\crysf_i \crysf^2_j \crysf_i(Z) = \crysf_j \crysf^2_i \crysf_j(Z) \neq \emptyset$.

\item We have the dual axiom, where the $\crysf_i$ above are replaced with $\cryse_i$,
and  the $\epsilon_i$ are replaced with $\phi_i$.
\end{enumerate}
These are reworded versions of Stembridge's local axioms given in \cite{Stembridge2003}.
...
In \cite{GillespieLevinson2019}, a Sembridge-type set of local axioms are used to define a type $B$ crystal structure.
...
In the follow-up paper, M. Gillespie, J. Levinston and K. Purbhoo \cite{GillespieLevinsonPurbhoo2020}
