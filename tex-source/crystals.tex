...
The notion of \defin{crystals} (in type $A$) refers to a set of \emph{raising}- and \emph{lowering operators} that give rise to a
certain graph structure on combinatorial objects (words, tableaux) with weights.
If the operators satisfy certain axioms, each connected component in the graph is a single Schur function.
...
In this video, \href{https://www.youtube.com/watch?v=qr7BcV3swMk}{\name{Jonah Blasiak} gives a nice overview}
...
This is referred to as an \defin{$i$-string}.
For example, 
\[
...
meaning that they commute with \hyperref[jeuDeTaquin]{jeu-de-taquin slides}.
...
Moreover, a Kashiwara crystal is a type $A$ \defin{Stembridge crystal} if 
...
\defin{Temperley--Lieb crystals}, built from shuffle tableaux and
