...
algebraic combinatorics: planar networks,
\hyperref[polyaFrequencySequenceDefinition]{Pólya frequency sequences}, real
...
If \(M_{i,j}\) is the weighted path sum from source \(s_i\) to sink \(t_j\),
then a \(2\times2\) minor has the form
\[
  M_{i,a}M_{j,b}-M_{i,b}M_{j,a}.
\]
The LGV involution cancels the intersecting path pairs, leaving only
nonintersecting pairs.  In a planar compatible network, the remaining weights
are nonnegative.  This is the local mechanism behind total nonnegativity of
the whole path matrix.
...
\item network proofs of \hyperref[realRootedPolynomial]{real-rootedness} and
...
\subsection[hankelTotalPositivityContinuedFractions]{Hankel matrices and continued fractions}

A polynomial sequence \((a_n)_{n\geq0}\) is
\defin{coefficientwise Hankel-totally nonnegative}\label{hankelTNN}
if every minor of the Hankel matrix
\[
  \bigl(a_{i+j}\bigr)_{i,j\geq0}
\]
has nonnegative coefficients.  Stieltjes and Jacobi continued fractions often
prove this property by realizing \((a_n)\) as the moments of weighted paths.

\name{Chao Xu} and \name{Jiang Zeng} derive Jacobi continued fractions for
multivariate \((p,q)\)-refinements of bi-Stirling Eulerian polynomials
\cite{XuZeng2026x}.  Their weighted Motzkin-path models give gamma-positive
expansions and prove coefficientwise Hankel total nonnegativity for two
alternating-permutation families and for a generalized Eulerian family
\cite[Thms.~7.2, 7.4, and~7.7]{XuZeng2026x}.
