...
The skew-hive extension of this model is described on the
\hyperref[skewHives]{hive-polytopes page} \cite{LeNguyen2026x}.

\name{Igor Pak} and \name{Daniel Soskin} study the largest coefficient
\[
 C(n)=\max_{\substack{|\lambda|=n\\|\mu|+|\nu|=n}}
 c^\lambda_{\mu,\nu}.
\]
They prove that at least one maximum is attained with
$\mu\subseteq\nu\subseteq\lambda$ and with $\nu/\mu$ a disjoint union of
single boxes \cite{PakSoskin2026x}.  They conjecture that every maximum has
this form for $n\geq16$, and verify the conjecture computationally through
$n=45$.
