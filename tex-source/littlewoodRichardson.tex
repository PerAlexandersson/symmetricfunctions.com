...
skew \hyperref[prelimTableaux]{semistandard Young tableaux} of shape $\lambda/\mu$ with content $\nu$,
...
$c^{\lambda}_{\mu\nu}$ is the number of \hyperref[prelimTableaux]{semistandard Young tableaux} $T$
...
In \cite{FominGreene1993,MolevSagan1999}, the following model is described.
$c^{\lambda}_{\mu\nu}$ is the number of \hyperref[prelimTableaux]{semistandard Young tableaux} $T$
with shape $\mu$, with the property that
...
The exact definition is written in an accessible manner in \cite[Def. A1.3.10]{StanleyEC2}.
...
A nice overview of the saturation conjecture is given by \name{Anders Buch} \cite{Buch2000}.
...
objects to compute structure constants in $K(\operatorname{Gr}(d,n))$, in
the localized cohomology of cotangent bundles of Grassmannians, and in the
cohomology of partial flag varieties.

\name{Portia Anderson} extends triangular Schubert puzzles to puzzles with
convex polygonal boundary shapes \cite{Anderson2024x}.  The resulting
four-, five-, and six-sided puzzles have Schubert-calculus interpretations,
...
with the \hyperref[littlewoodRichardsonFromKostant]{Littlewood--Richardson coefficients from the Kostant partition function}.
...
The \defin{Newell--Littlewood numbers} may be defined as
\[
...
The Newell--Littlewood numbers arise in the same manner, where $\GL(V)$ is replaced by a Lie group of type $B$, $C$ or $D$.
...
Computer experiments, see \cite{KingTolluToumazet2004}, suggest that the Ehrhart polynomial above has non-negative coefficients.
...
it is proved by \name{Christian Ikenmeyer} \cite{Ikenmeyer2016} and later \name{Cass Sherman} \cite{Sherman2015x,Sherman2017} that
\[
...
This preprint contains lots of interesting conjectures analogous to results/conjectures about the Littlewood--Richardson
...
\name{Hariharan Narayanan} proved in 2006 that computing Littlewood--Richardson coefficients and 
