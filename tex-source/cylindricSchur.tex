...
of such cylindric Schur functions are given by 3-point Gromov--Witten invariants of the Grassmannian.
...
Note that the second example shows that the \hyperref[schurSkew]{skew Schur functions} are a proper subset
...
\name{Alexander Postnikov} uses the notation $\lambda/d/\mu$ to describe a cylindric shape,
...
For the poset structure on cylindric diagrams and connected
to \hyperref[rootSystem]{root systems} and the Bruhat order,
...
fundamental quasisymmetric basis, but \hyperref[cylindricSchurPositivity]{\enquote{most} cylindric Schur functions are not}.
...
and conjectures that they decompose into irreducible \hyperref[spechtModules]{Specht modules} with
...
The $C_{\mu,\nu}^{\lambda,d}$ are closely related to \defin{fusion coefficients}
...
\name{Peter McNamara} \cite{McNamara2006} asked for which cylindric shapes $\schurCylindric_{\lambda/d/\mu}(\xvec)$ are
...
\name{Peter McNamara} proves that only ordinary skew Schur functions are \hyperref[schurPositivity]{Schur-positive}.
...
For a short proof, see \cite{Dobner2026x} where the structure coefficients are interpreted using \emph{fusion coefficients}.
