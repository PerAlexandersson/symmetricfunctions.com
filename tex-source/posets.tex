...
$x\lt_P y$ if $x\leq_P y$ and $x\neq y$.  A \defin{cover relation} is a
relation $x\lessdot_P y$ such that $x\lt_P y$ and there is no $z$ with
$x\lt_P z\lt_P y$.  The \defin{Hasse diagram} of $P$ draws only these cover
relations.

Two basic examples are the \defin{chain} $1\lt 2\lt \dotsb \lt n$ and the
\defin{antichain} on $n$ elements, where no two distinct elements are
comparable.  Posets are useful because many combinatorial objects are naturally
sets with precedence constraints: boxes in a Young diagram, roots in a root
poset, subsets ordered by inclusion, or independent sets ordered by inclusion.
...
\defin{antichain}\label{antichain} is a subset whose elements are pairwise
incomparable.
...
On order ideals, one important action is rowmotion: send an ideal to the ideal
generated by the minimal elements outside it.  Root posets and minuscule
posets give many cyclic-sieving examples; see the
\hyperref[cspListPosetsRoot]{root poset} and
\hyperref[cspListPosetsMinuscule]{minuscule poset} sections on cyclic
sieving.

\todo{Add a four-element rowmotion orbit example on order ideals, and decide
whether rowmotion deserves its own stable definition label.}
...
induced subposet consisting of two disjoint two-element chains.  Posets that
are both $(3+1)$-free and $(2+2)$-free are unit interval orders, and their
incomparability graphs are unit interval graphs.  This is the setting of many
