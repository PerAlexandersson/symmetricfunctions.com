...
elements, in which no two distinct elements are comparable.  Posets are useful
because many combinatorial objects are naturally sets with precedence
constraints: boxes in a \hyperref[youngDiagram]{Young diagram}, roots in a
root poset, subsets ordered by inclusion, or independent sets ordered by
inclusion.
...
\defin{Jordan--Hölder set} records instead the
...
\defin{$P$-Eulerian polynomial} is the descent
...
\begin{example*}[Relabeled Jordan--Hölder sets]
...
\begin{example*}[Small naturally labeled posets]
The polynomial \(W_P(t)\) records the descent distribution of the linear
extensions; it is not the same data as the poset itself.

\begin{tabular}{lll}
\toprule
Poset \(P\) & Linear extensions & \(W_P(t)\) \\
\midrule
Chain \(1\lt 2\lt 3\) & \(123\) & \(1\) \\
\(1,2\lt 3\) & \(123,213\) & \(1+t\) \\
\(1\lt 3\), with \(2\) isolated & \(123,132,213\) & \(1+2t\) \\
Antichain on \(\{1,2,3\}\) & \(S_3\) & \(1+4t+t^2\) \\
\bottomrule
\end{tabular}
\end{example*}

\section[posetChainPolynomials]{Chain polynomials}

The \defin{chain polynomial}\label{chainPolynomial} of a finite poset $P$ is
\[
 f_P(t)=\sum_{i\geq0}c_i(P)t^i,
\]
where $c_i(P)$ is the number of $i$-element chains in $P$; equivalently,
$f_P$ is the $f$-polynomial of the order complex $\Delta(P)$.

\begin{theorem}[Athanasiadis--Douvropoulos--Kalampogia-Evangelinou]
Let $P$ be a Cohen--Macaulay simplicial poset of rank $n$, let $\widehat P$
be obtained by adjoining a maximum element, and for $T\subseteq[n]$ set
\[
 \widehat P_T=\{y\in P:\operatorname{rank}(y)\in T\}
 \cup\{\widehat0,\widehat1\}.
\]
Then $f_{\widehat P_T}(t)$ is real-rooted.  Moreover,
\[
 h(\Delta(\widehat P_T),t)
 \interl A_n^{T\cap[n-1]}(t),
\]
where $A_n^T$ is the
\hyperref[restrictedEulerianPolynomial]{restricted Eulerian polynomial}
\cite[Thm.~1.2]{AthanasiadisDouvropoulosKalampogiaEvangelinou2024}.

For every finite Coxeter group $W$, the chain polynomial of its noncrossing
partition lattice $NC_W$ is real-rooted.  If $W$ is irreducible, then
$h(\Delta(NC_W),t)$ also has a nonnegative real-rooted symmetric
decomposition
\cite[Thm.~1.3]{AthanasiadisDouvropoulosKalampogiaEvangelinou2024}.
\end{theorem}

A restricted type-$D$ enumeration of chains proceeding by steps of type
$A_2$, with at most one final step of type $A_1$, is \oeis{A363861}.
...
a graph $G$, the independent sets form a down-set in the
\hyperref[booleanLattice]{Boolean lattice} $2^{V(G)}$.  This poset is usually
not a lattice under union, but it is a natural way to turn graph-theoretic
independence into an ordered object.

\section[posetHookFormulas]{Hook formulas}

Many important posets have unexpectedly simple formulas for $e(P)$.

For a partition $\lambda$, the boxes of its \hyperref[youngDiagram]{Young
diagram} form a poset by declaring a box to be less than the boxes weakly
southeast of it.  A \hyperref[standardYoungTableau]{standard Young tableau}
of shape $\lambda$ is the same thing as a linear extension of this
Young-diagram poset.  The classical
...
There is an analogous hook formula for
\defin{rooted tree posets}\label{rootedTreePoset}.  If $T$ is a rooted tree
...
Posets also carry natural dynamics.  The dedicated
\hyperref[rowmotionPromotion]{rowmotion and promotion page} develops
Schützenberger promotion on linear extensions, rowmotion on order ideals, and
their descriptions through toggles.  It includes illustrated small examples,
the promotion--rowmotion conjugacy for rowed-and-columned posets, and recent
cyclic-sieving and homomesy results for Tamari lattices.

For arbitrary labelings, \name{Colin Defant} and \name{Noah Kravitz}
introduce promotion sorting \cite{DefantKravitz2022}.  Promotion is also one
of the standard cyclic actions behind \hyperref[cyclicSieving]{CSPs} for
\hyperref[cspRectangularSYT]{rectangular standard Young tableaux} and
\hyperref[cspKrewerasWords]{Kreweras words}.

\section[posetClasses]{Some classes of posets}

The \defin{width}\label{posetWidth} of a poset is the largest size of an
\hyperref[antichain]{antichain}.  Width-one posets are chains; width-two
posets already contain rich examples such as the posets attached to skew
Ferrers diagrams and \hyperref[latticePathMatroids]{lattice path matroids}.

A \defin{ranked poset}\label{rankedPoset} is a poset with a rank function
\(\rho:P\to\setN\) such that \(x\lessdot_P y\) implies
\(\rho(y)=\rho(x)+1\).  A poset is
\defin{graded}\label{gradedPoset} if every maximal chain in every interval has
the same length; finite graded posets are ranked after choosing the rank of
each minimal element.
...
The \defin{Boolean lattice}\label{booleanLattice} \(B_n\) is the lattice of
all subsets of \([n]\), ordered by inclusion.  A
\defin{distributive lattice}\label{distributiveLattice} is a lattice in which
meet and join distribute over one another.  The lattices \(J(P)\) of order
ideals are distributive, and Birkhoff's representation theorem says that
every finite distributive lattice arises this way; see \cite{StanleyEC1}.

A \defin{series-parallel poset}\label{seriesParallelPoset} is built from
one-element posets using two operations:
\hyperref[disjointUnionPoset]{disjoint union}, also called parallel
composition, and \hyperref[ordinalSumPoset]{ordinal sum}, also called series
composition.  Such posets are often amenable to recursive enumeration because
both operations have simple effects on linear extensions.  These operations
preserve real-rootedness of $P$-Eulerian polynomials; see the
\hyperref[posetRealRooted]{real-rootedness discussion}.
...
\begin{example*}[$(3+1)$-free and not $(3+1)$-free]
Every poset of height at most two is automatically $(3+1)$-free, since it has
no three-element chain.  For example, the diamond-shaped poset with
relations \(a,b\lt c,d\) is $(3+1)$-free.

In contrast, take a chain \(a\lt b\lt c\) and add an element \(d\) that is
incomparable to \(a,b,c\).  The induced subposet on
\(\{a,b,c,d\}\) is exactly a \((3+1)\), so this poset is not
$(3+1)$-free.
\end{example*}
