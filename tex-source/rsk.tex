...
Given a SSYT $T$ and $k \in \setN$, the \emph{insertion}
...
the Robinson--Schensted algorithm for skew Young tableaux
...
the FPSAC 2020 talk
\url{https://www.youtube.com/watch?v=SeFLyM3zDV0}.
...
\name{Amanda Burcroff} and \name{Colin Defant} use RSK to study strong pattern
avoidance, where both a permutation and one of its powers avoid a fixed pattern
\cite{BurcroffDefant2020}.



In \cite{Pahuja2026x}, the author considers matrices that map to a fixed shape
$\lambda$ under RSK and studies those with the minimal number of inversions,
computed from the second row in the biword.  The author conjectures that these
\emph{minimal matrices} are exactly the symmetric Hankel matrices.
...
A different bijection is defined by \name{Igor Pak} \cite{Pak2001}, from reverse plane partitions to $\lambda$-arrays.
...
\defin{Domino insertion} is a type $B/C$ analog of RSK for signed
...
\name{Alexander Yong} develop a $K$-theoretic jeu-de-taquin theory for
...
In a more recent paper, they define a $qt$-version of RSK from which one can prove the dual Cauchy identity for 
...
functions.
See \cite{CorwinOConnellSeppalainenZygouras2014}.
...
correspondence and a super Cauchy identity, see \name{James Rowan}'s notes
\url{https://math.mit.edu/research/undergraduate/urop-plus/documents/Rowan.pdf}.
...
from the viewpoint of geometric crystals and invariant theory
...
Richardson tableaux of size $n$ and Motzkin paths with $n$ steps, and connects
...
tableaux as a subset of standard tableaux motivated by Springer fibers and
totally nonnegative geometry \cite{KarpPrecup2025x}.  They characterize these
tableaux using evacuation and pairs of reading words, prove that the
corresponding Springer-fiber component is a Richardson variety, and enumerate
