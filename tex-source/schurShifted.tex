...
- Misc shiftedSchur -> schurS (current guess: schurShifted -> schurS); from polynomialData.m, tikzdataRelations.txt.
}
...
\todo{Legacy relation hints from old polynomialData.m/tikzdata*.txt to verify or migrate:
- SpecializesTo doubleSchur -> schurS (current guess: schurFactorial -> schurS); from polynomialData.m, tikzdataRelations.txt.
- SpecializesTo factorialSchur -> doubleSchur (current guess: schurFactorial -> schurFactorial); from polynomialData.m, tikzdataRelations.txt.
}
...
They were introduced by \name{Lawrence Biedenharn} and \name{James Louck} in \cite{BiedenharnLouck1989}.
...
The top-degree component is the classical \hyperref[schurS]{Schur polynomial} $\schurS_\lambda$. 
...
\hyperref[prelimTableaux]{semi-standard Young tableaux} of shape $\lambda$ and maximal entry at most $n$):
\[
...
Under the \hyperref[gtpatternsAsSSYT]{GT-pattern to SSYT bijection}, the quantity $m_{i,j}-m_{i,j-1}$ is sent 
