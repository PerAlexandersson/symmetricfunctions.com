...
They are enumerated by the ordered Bell, or Fubini, numbers
\oeis{A000670}.
...
The permutations of type $B$ are the permutations of
$B_n\coloneqq\{\pm1,\pm2,\dotsc,\pm n\}$ satisfying
$\pi(-i)=-\pi(i)$.  They form the
\hyperref[weylLieGroups]{hyperoctahedral group}, which has order
$2^n n!$; see \oeis{A000165}.
...
a Schur-positivity result for the corresponding
\hyperref[ex:typeBEulerianSturm]{descent enumerators}.
...
\]
In particular, there are $r^n n!$ colored permutations of size $n$.
For example,
\[
  3^{(2)}\,1^{(0)}\,2^{(1)}
\]
...
\hyperref[descentSet]{descents} and \hyperref[majorIndex]{major index}, have
colored analogues; see
...
These are permutations of the multiset $\{1,1,2,2,3,3,\dotsc,n,n\}$ with the
additional property that between the two entries equal to $i \in [n]$, only
entries larger than $i$ may appear.
For example, $12234431$ is an element in $Q_n$.

The cardinality is $|Q_n|=(2n-1)!!$; see \oeis{A001147}.
