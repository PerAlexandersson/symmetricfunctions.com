...
In \cite{LiangBowling2017x}, matchings on $[2n]$ with $k$ crossings are studied.
Let $X_{n,k}$ be the set of such matchings, and let $\grpc_{2n}$ act on such matchings via rotation.
The case $k=0$ corresponds to the \hyperref[cspNonCrossingMatchingsI]{Catalan case}.
...
In \cite{Kim2013}, a generalization of non-crossing permutations is studied.
...
The paper \cite{Kim2013} only considers non-crossing permutations with at least one connected cycle.
...
Several refinements, such as the $q$-Narayana and $q$-Kreweras polynomials are also considered
...
\name{Stefan Kluge} proves the cyclic sieving phenomenon for non-crossing
forests, confirming a conjecture of \name{Alan Guo} \cite{Kluge2012}.
...
the following $q$-analog is considered, which enumerates the number of
