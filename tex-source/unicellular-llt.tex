...
\metakeywords{unicellular LLT polynomial, LLT polynomial, unit interval graph,
chromatic quasisymmetric function, Frobenius characteristic, twin manifold,
dagger action}
...
The \defin{unicellular LLT polynomials}\label{unicellularLLTPolynomialDefinition}
are the one-cell-per-component
...
description, observed by \name{Erik Carlsson} and \name{Anton Mellit}, and
independently in joint work with \name{Greta Panova}, uses
\hyperref[chromaticQuasisymmetricUnitIntervalGraph]{unit interval graphs} and
their area sequences \cite{CarlssonMellit2017,AlexanderssonPanova2018}.
...
an edge $\{i,j\}$ with $i\lt j$ is an
\defin{ascent}\label{unicellularLLTAscent} of $\kappa$ if
$\kappa(i)\lt\kappa(j)$.
We write $\asc(\kappa)$ for the number of such edges.  The
\defin{unicellular LLT polynomial}\label{unicellularLLTGraphPolynomial} is
\[
\LLT_\avec(\xvec;q)
\coloneqq
\sum_{\kappa:[n]\to\setP}
...
The complete graphs on $n$ vertices are
\hyperref[unitIntervalGraph]{unit interval graphs}.  For the complete graph
$K_n$, one has
\[
\LLT_{K_n}(\xvec;q)
=
...
In joint work with \name{Greta Panova}, we prove that for any area sequence
$\avec$ of length $n$,
\[
...
These traces are closely connected with
\hyperref[kazhdanLusztigPolynomials]{Kazhdan--Lusztig polynomials}.


\subsection[unicellularLLTRecursions]{Recursions}

Several recursions and linear relations are known for unicellular LLT
polynomials.  For example, \name{Seung Jin Lee} gives a three-term recursion
in \cite{Lee2020}.  Short bijective proofs of these recursions are given in
...
It is a major open problem to find a combinatorial rule for the
\hyperref[schurPositivity]{Schur expansion} of $\LLT_\avec(\xvec;q)$.  For
so-called melting lollipop graphs, \name{JiSun Huh}, \name{Seung Jin Nam}, and
\name{Meesue Yoo} give a combinatorial formula for the Schur expansion
\cite{HuhNamYoo2020}.
...
skew shapes, $\LLT_{\nuvec}(\xvec;q)$ is
\hyperref[schurPositivity]{Schur-positive} with an explicit combinatorial
formula.  In the unicellular case, this includes the area
...
stronger than \hyperref[schurPositivity]{Schur positivity} \cite{Miller2019}.
The bandwidth-at-most-two case was treated earlier by
\name[S.J. Lee]{Seung Jin Lee} \cite{Lee2020}.

\name{Soojin Cho} and \name{JiSun Huh} give a combinatorial formula for the
hook coefficients, that is, the coefficients of $\schurS_{\lambda}$ where
...
$\LLT_\avec(\xvec;q)$ in the \hyperref[powerSum]{power-sum basis}.
...
order-preserving surjections $f:P\to[k]$ for some $k$.  The
\defin{type}\label{orderPreservingSurjectionType} of such a surjection is
\[
...
\cite[Prop.~5.2]{BernardiNadeau2020}.


\subsection[unicellularLLTEExpansion]{Elementary symmetric expansion}

Together with \name{Greta Panova}, we conjectured in
\cite{AlexanderssonPanova2018} that for any
\hyperref[unitIntervalGraph]{unit interval graph} $\avec$, the polynomial
$\LLT_\avec(\xvec;q+1)$ is positive in the
\hyperref[elementaryE]{elementary symmetric function basis}.  The formula
below was later proved by \name{Per Alexandersson} and
\name{Robin Sulzgruber}.

Let $\theta\in O(\avec)$, and for each $i\in[n]$ define the
\defin{highest reachable vertex}\label{highestReachableVertex} by
\[
...
polynomials.  The path graph case appears in
\cite{AlexanderssonPanova2018}, and the complete graph case follows from a
recursion.  For melting lollipop graphs, \cite{Alexandersson2019llt} gives a
similar-looking $\elementaryE$-expansion.
...
A refinement to LLT cumulants imposes certain connectivity requirements
\cite{Kowalski2020x}; see also \cite{DolegaKowalski2021x}.
...
The polynomial $\LLT_\avec(\xvec;q)$ is the
...
The polynomial $\LLT_\avec(\xvec;q)$ is symmetric and
\hyperref[schurPositivity]{Schur-positive}.
Hence it can be interpreted as a
\hyperref[frobeniusCharacteristic]{graded Frobenius characteristic}:
\[
\LLT_\avec(\xvec;q)
=
...
\LLT_\avec(\xvec;q).
\]
This is the LLT analogue of the
\hyperref[dotAction]{Hessenberg dot-action theorem}.
...
A complementary finite-group interpretation is due to \name{Lucas Gagnon}.
He realizes vertical-strip LLT polynomials by inducing characters from the
unipotent upper triangular group to $\GL_n(\mathbb{F}_q)$ and then recording
the unipotent constituents \cite[Thm.~5.1]{Gagnon2024}.


\subsection[unicellularLLTTwinGKM]{The twin GKM model}

The twin-manifold GKM model is close to the regular semisimple
\hyperref[hessenbergVarieties]{Hessenberg} GKM model, with two differences.
...
Second, the group action is the \defin{dagger action}\label{daggerAction}.
...
p_{\sigma^{-1}v}.
\]
This should be contrasted with \hyperref[tymoczkoDotAction]{Tymoczko's dot
action}, where $\sigma$ also acts on the variables $t_1,\dotsc,t_n$.
