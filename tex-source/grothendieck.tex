...
There is another version of the Grothendieck polynomials that comes from the connective K-theory that involves another variable $\beta$ (see \cite{FominKirillov1994grothendieck}) that is equivalent to the ones coming from the usual K-theory.
This can be seen for $\grothendieck_{\omega}(\xvec)$ by the substitution $x_i \mapsto -\beta x_i$ and multiplying by $(-\beta)^{-\ell(\omega)}$, noting that $\ell(\omega)$ is the lowest degree component of $\grothendieck_{\omega}(\xvec)$.
...
\subsection[grothendieckBumplesspipedreams]{Bumpless pipe dreams}
...
There is an expression for Grothendieck polynomials using alternating sign matrices, due to \name{Alain Lascoux}.
This is essentially a translation of the bumpless pipe dream formula into its essential information.
...
\section[biaxialGrothendieck]{Biaxial Grothendieck polynomials}

\begin{polydata}{grothendieckStable}
  Name   & Biaxial Grothendieck polynomials \\
  Space    & All \\
  Basis    & False \\
  Rating   & 1 \\
  Bib      & BrubakerFrechetteHardtTiborWeber2020x \\
  Year     & 2020 \\
  Symbol   & $\grothendieck_{\omega,\nu}^{(\beta,q)}(\xvec; \yvec)$ \\
  Category & Schubert \\
\end{polydata}


In \cite{BrubakerFrechetteHardtTiborWeber2020x}, the authors introduce biaxial Grothendieck polynomials, which generalize the double Grothendieck polynomials as well as the double dual Grothendieck polynomials.
Their construction is taking a (colored) solvable lattice model for pipe dreams and replacing it with the corresponding quanum group R-matrix version of the Boltzmann weights.
Thus the biaxial Grothendieck polynomials are the partition function for this lattice model, which specializes to the double Grothendieck polynomials and the duals.
...
They serve as a K-theoretic analog of the \hyperref[stanleySymLimitDefinition]{Stanley symmetric functions} as the stable Grothendieck polynomials are defined as the limit 
\begin{equation*}
 \grothendieckStable_\omega(\xvec) \coloneqq \lim_{k\to \infty}  \grothendieck_{1^k \times \omega}(\xvec).
\end{equation*}
Like for Stanley symmetric functions, stable Grothendieck polynomials are symmetric polynomials.
...
Stable Grothendieck polynomials indexed by Grassman permutations form a basis of symmetric functions.
We will refer to these as the \defin{Grassman Grothendieck polynomials} or \defin{symmetric Grothendieck polynomials} to distinguish them from the larger class of stable Grothendieck polynomials
(although some places in the literature does not make this distinction).
They are the K-theoretic analog of the \hyperref[schurS]{Schur functions} and have many analogous properties and constructions.
...
\end{align}
The set $\mathrm{SVT}(\lambda)$ consists of all set-valued tableaux of shape $\lambda$, where each box is filled with a non-empty set of natural numbers.
...
C. Lenart \cite[Thm. 2.8]{Lenart2000} proved that whenever $k\geq \length(\lambda)$,
\[
...
\subsection[grothendieckSymmetricLittlewoodRichardson]{Littlewood--Richardson rule}

There is a Littlewood--Richardson rule for the product of symmetric Grothendieck polynomials:
\[
...
\subsection[grothendieckSkewSymmetric]{Skew symmetric Grothendieck}


In \cite{Buch2002,LiMorseShields2016}, 
the skew version of the symmetric Grothendieck polynomials are defined as
\begin{align}
\grothendieckStable_{\nu/\lambda}(\xvec) = \sum_{\mu} 
\sum_{T \in \mathrm{SVT}(\nu/\lambda,\mu)} (-1)^{|\lambda|-|\nu|-|\mu|} \monomial_\mu(\xvec).
\end{align}

From a result in \cite{BilleyJockuschStanley1993}, it follows that the skew symmetric Grothendieck polynomials 
are precisely the stable limit of Grothendieck polynomials indexed by 321-avoiding permutations.


A recent result in \cite{ChanPflueger2019} gives an expansion of the skew symmetric Grothendieck polynomials
...
the expansion of stable Grothendieck polynomials into Grassman (or symmetric) Grothendieck polynomials.
...
Describe a \hyperref[crystals]{crystal structure}, whose connected components give symmetric Grothendieck polynomials.
This would be analogous to the type $A$ crystals that produce Schur polynomials.
\end{problem}

This problem has been essentially answered in a recent preprint of Marberg, Tong, and Yu \url{https://arxiv.org/pdf/2501.16640.pdf}.



\section[grothendieckDualStable]{Dual symmetric Grothendieck polynomials}
...
The dual symmetric Grothendieck polynomials were first studied in \cite{LamPylyavskyy2007combinatorialhopf}.
...
the \hyperref[grothendieckStableLittlewoodRichardson]{Littlewood--Richardson rule for symmetric Grothendieck polynomials}.


\subsection[grothendieckDualStableBialternant]{Bialternant formula}


A \defin{bialternant formula} for the dual symmetric Grothendieck polynomials 
...
\todo{ Add ref to refined dual symmetric JT-identity.}

There is also a Jacobi--Trudi identity for the \defin{skew dual symmetric Grothendieck polynomials},
originally conjectured by \name{Darij Grindberg} \href{http://www.cip.ifi.lmu.de/~grinberg/algebra/chicago2015.pdf}{(see these slides)}. 
...
of \defin{refined dual symmetric Grothendieck polynomials},
...
\section[grothendieckCanonical]{Canonical Grothendieck polynomials}

\begin{polydata}{grothendieckCanonical}
  Name     & Canonical Grothendieck polynomials \\
  Space    & Sym \\
  Basis    & True \\
  Rating   & 2 \\
  Bib      & Yeliussizov2017 \\
...
In \cite{Yeliussizov2017}, \name{Damir Yeliussizov} introduced a two-parameter deformation of the (one-parameter) symmetric Grothendieck polynomials
$\grothendieckStable^{(a,b)}_\lambda(\xvec)$, which we call the \defin{canonical Grothendieck polynomials} (we drop other adjectives for simplicity as there is no ambiguity),
and the dual symmetric Grothendieck polynomials $\grothendieckDual^{(a,b)}_\lambda(\xvec)$, which we call the \defin{dual canonical Grothendieck polynomials}, with the following properties.
...
\item Highest (resp. lowest) degree term is a Schur polynomial,
\[
...
\item 
They have the same structure constants $c^{\nu}_{\lambda\mu}$ as the usual symmetric Grothendieck polynomials,
\[
...
the canonical symmetric Grothendieck polynomials is given by G. Hawkes and T. Scrimshaw \cite{HawkesScrimshaw2019}.


\begin{problem}[See \cite{HawkesScrimshaw2019}]
Construct a crystal on some combinatorial model for dual canonical Grothendieck polynomials $\grothendieckDual^{(a,b)}_{\mu}$.
\end{problem}

An ideal solution to this problem would be the crystal specialize to the crystals in \cite{Galashin2017} (resp. \cite{HawkesScrimshaw2019}) for the specialization $b = 0$ (resp. $a = 0$).
...
\section[grothendieckRefinedDualStable]{Refined dual symmetric Grothendieck polynomials}


\begin{polydata}{grothendieckRefinedDualStable}
  Name   & Refined dual symmetric Grothendieck polynomials \\
  Space    & Sym \\
  Basis    & True \\
  Rating   & 1 \\
...
The \defin{refined dual symmetric Grothendieck polynomial} were introduced in \cite{GalashinGrinbergLiu2016}.
...
In \cite{Yeliussizov2019}, the \defin{generalized dual symmetric Grothendieck polynomials}
are defined:
\[
...
where $\grothendieckRefinedDualStable_\lambda$ is a refined dual symmetric Grothendieck polynomial.
...
\]
Yeliussizov proves several new $q$-identites involving 
...
is the refined dual symmetric Grothendieck polynomial. 
They give numerous new identities
and new proofs of the Jacobi--Trudi identities for the skew refined dual symmetric Grothendieck polynomials.
Moreover, a new Cauchy identity is given as well.
This article has a lot of interesting new identities and connections.
This also naturally generalized the \hyperref[dualGrothendieckPercolation]{last-passage percolation} from \cite{Yeliussizov2020}, where the generalized dual symmetric Grothendieck polynomials are (up to an overall normalization factor) the multipoint probabilities.




\section[grothendieckRefinedSymmetric]{Refined symmetric Grothendieck polynomials}


\begin{polydata}{grothendieckRefinedSymmetric}
  Name   & Refined symmetric Grothendieck polynomials \\
  Space    & Sym \\
  Basis    & True \\
  Rating   & 1 \\
  Bib      & ChanPflueger2019 \\
  Year     & 2019 \\
  Symbol   & $\grothendieckRefinedStable_\lambda(\xvec)$ \\
  Category & Schur \\
  Keywords & schur-positive
\end{polydata}

These first appeared in the work on M. Chan and N. Pflueger \cite{ChanPflueger2019}, where they refined the set-valued tableaux formula.
Solvable lattice models for these polynomials were discussed at the end of~\cite{MotegiScrimshaw2020x}.




\section[grothendieckRefinedCanonical]{Refined canonical Grothendieck polynomials}
...
\begin{polydata}{grothendieckRefinedSymmetric}
  ...
  Bib      & HJKSS24 \\
  Year     & 2019 \\
  Symbol   & $\grothendieckRefinedCanonicalStable_\lambda(\xvec)$ \\
  Category & Schur \\
  Keywords & schur-positive
\end{polydata}


By combining the refined symmetric Grothendieck polynomials and canonical Grothendieck polynomials, B.-H. Hwang et al. introduced the currently most general form of the symmetric Grothendieck polynomials in \cite{HJKSS24,HJKSS25}.
We call these the \defin{refined canonical Grothendieck polynomials}.
In these papers, they also introduced the \defin{dual refined canonical Grothendieck polynomials}, and for these functions, they provided the corresponding combinatorial formulas, Jacobi--Trudi formulas, Schur expansions, and the Cauchy identity.
S. Iwao, K. Motegi, and T. Scrimshaw gave another construction for these functions was given in terms of the boson-fermion correspondence in \url{https://arxiv.org/pdf/2211.05002} and connected these functions with certain stochastic processes in \url{https://arxiv.org/abs/2311.01116}.
