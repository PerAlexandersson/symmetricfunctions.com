...
\metadescription{A guide to elementary-basis expansions of chromatic symmetric and chromatic quasisymmetric functions, 
including the Stanley--Stembridge and Shareshian--Wachs conjectures, q-refined results, 
...
\]
We say that $\chrom_G(\xvec)$ is \defin{$\elementaryE$-positive} if all
...
\hyperref[chromaticQuasisymmetricUnitIntervalGraph]{unit interval graphs} or circular unit arc digraphs.
...
The original conjecture was formulated by \name{Richard Stanley}
and \name{John Stembridge} \cite{StanleyStembridge1993}, and then in the
language of chromatic symmetric functions by Stanley \cite{Stanley1995}.
...
Hikita later introduced \defin{$(q,t)$-chromatic symmetric functions} for
...
This unimodality conjecture also extends to the circular 
unit arc digraph setting.
...
noncommuting variables, as in \cite{GebhardSagan2001}. This implies ordinary
...
\item
\emph{2024--2026.}
Recent results include the $2+1+1$-avoiding unit interval orders
...
\item
The $2+1+1$-avoiding unit interval orders, equivalently the area sequences
with $i-2 \leq a_i$, are treated in
...
\begin{example}[The smallest star obstruction]
...
acyclic orientations and sinks. The change-of-basis perspective in
