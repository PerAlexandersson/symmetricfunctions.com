...
Entries $T^i(u) > T^j(v)$ in a tuple form an \defin{inversion} if
either 
\begin{itemize}
...
If $F$ is a filling of the diagram, we have for example inversions if $F(1) < F(2)$ or $F(2) < F(3)$.
...
graph is triangle-free (see the section on unicellular LLT polynomials). \name{Foster Tom} uses a generalization of cocharge.
...
The unicellular LLT polynomials are a subset of LLT polynomials indexed
...
and we recommend the reader to first consult that section.
...
\]
Note that $\macdonaldH_{(n)}(\xvec;q,t)$ is a \hyperref[macdonaldH]{modified Macdonald polynomial} 
...
\[
\omega \LLT_{\avec}(\xvec;q) = q^{(a_1+a_2+\dotsb+a_n)} \LLT_{\avec^T}(\xvec;1/q)
\]
...
The unicellular LLT polynomials are also evaluations of certain traces of \hyperref[heckeAlgebra]{Hecke algebras},
see \cite{MoralesSkanderaWang2024}. There is a close connection with Kazhdan--Lusztig polynomials.
...
For example, \name{Seung Jin Lee}'s three term recursion in \cite{Lee2020}.
We provide short bijective proofs of \name{Seung Jin Lee}'s recursions in \cite{Alexandersson2019llt}.

Lee's recursions are generalized further by \name{Christopher Miller}, see \cite[Eq. (2.2)]{Miller2019}.
...
Since LLT polynomials are $q$-deformations of \hyperref[schurLittlewoodRichardson]{Littlewood--Richardson coefficients}, see \cite{LeclercThibon2000},
...
There is a statistic $c_\avec(T)$ on \hyperref[prelimTableaux]{standard Young tableaux} depending on $\avec$, such that
\[
...
In his PhD thesis, \name{Christopher Miller} shows that LLT polynomials 
with bandwidth at most $3$ are \hyperref[kSchur]{$3$-Schur-positive},
which is stronger than Schur positivity, \cite{Miller2019}.
The bandwidth of size at most two was treated earlier by \name[S.J. Lee]{Seung Jin Lee} \cite{Lee2020}.


In \cite{ChoHuh2019}, a combinatorial formula for the \emph{hook coefficients} was provided.
These are coefficients of $\schurS_{\lambda}$ where $\lambda$ is of the form $(m,1,1,\dotsc,1)$.
...
Together with \name{Greta Panova}, we conjecture that for any unit-interval graph $\avec$,
...
In June 2019, \name[M. D'Adderio]{Michele D'Adderio} \cite{DAdderio2019x} posted a 
...
Together with \name{Robin Sulzgruber}, we proved the combinatorial formula below.
...
Another refinement is given by \name{Alex Abreu} and \name{Antonio Nigro} in \cite{AbreuNigro2021},
...
\name[J.-C. Novelli]{Jean-Christophe Novelli} and \name[J.-Y. Thibon]{Jean-Yves Thibon} \cite[Thm. 6.2]{NovelliThibon2019} 
...
relation with \hyperref[chromaticQuasisymmetricLLT]{chromatic quasisymmetric functions}.
