...
After choosing a monomial basis of $V$, the map $\rho$ sends a permutation $g \in G$ to a permutation matrix in $GL(V)$.
\end{example}
...
then we define the \defin{tensor product of representations} as the map
...
\]
where $f^\mu$ is the number of \hyperref[prelimTableaux]{standard Young tableaux} of shape $\mu$.
...
\defin{Schur--Weyl graph}, the branching graph attached to Schur--Weyl
...
We then have the following relation:
 ...
 Note that $\dim(S^{\lambda}) = f^\lambda$, the number of \hyperref[prelimTableaux]{standard Young tableaux} of shape $\lambda$.
...
\begin{example}
Symmetric group action on set-partitions and non-crossing set partitions:
\href{https://www.youtube.com/watch?v=IJ3tTGJTofQ}{this lecture}.
See also $SL_3$-webs.
\end{example}
...
\cite{Kamnitzer2009x}
and
\href{https://sbseminar.wordpress.com/2008/07/08/how-to-write-down-the-representations-of-gl_n/}{Speyer's
blog post on writing down representations of $\GL_n$}.
For broader lecture notes on representation theory, see
\href{https://www.math.upenn.edu/~zvihr/courses/RTNotes.pdf}{Rosen's notes}
and
\href{http://math.cmu.edu/~cnewstea/notes/reptheory.pdf}{Newstead's notes}.
...
For the interaction between representation theory and \hyperref[zeroHeckeAlgebra]{0-Hecke algebras}, see \cite{HicksMillerBrown2024x}.
...
\defin{generalized flagged Schur modules} and \defin{key modules} in their
...
In \cite{Szendroi2026x}, the author considers
the coordinate rings of general Segre embeddings of products of projective spaces.
Their bigraded Hilbert polynomials get expressed in terms of major-descent generating functions
of words in multisets. Also, quantum cohomology and Garsia--Stanton-style bases are mentioned.
