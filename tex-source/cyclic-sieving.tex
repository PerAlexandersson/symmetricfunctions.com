...
There are many instances of the CSP, which we have put into different categories
based roughly on the type of combinatorial object it concerns.
\begin{itemize}
\item \hyperref[cspCatalan]{CSP on Catalan objects.}
\item \hyperref[cspMatch]{CSP on matchings.}
\item \hyperref[cspPath]{CSP on paths (including lattice paths).}
\item \hyperref[cspTableau]{CSP on tableaux and fillings.}
\item \hyperref[cspWord]{CSP on words and permutations.}
\item \hyperref[cspMisc]{CSP on other objects.}
\end{itemize}


\subsection[cspDefinition]{Definition}

The \defin{cyclic sieving phenomenon} (CSP) was introduced by \name{Vic Reiner}, \name{Dennis Stanton} and 
\name{Dennis White} in \cite{ReinerStantonWhite2004}. 
A nice survey by \name{Bruce Sagan} is given in \cite{Sagan2011}.
...
is a \hyperref[qAnalogs]{$q$-analog} (or $q$-enumeration) of the set $X$.
The cyclic action need not be faithful.  If $\grpc_h$ acts with a kernel of
order $d$, then the CSP is equivalent to the faithful quotient action of
$\grpc_{h/d}$ together with a polynomial $f'(q)$ satisfying
$f(q)=f'(q^d)$; see \cite[Sec.~2.5]{BessisReiner2011}.

\name[Geng-Rui Zhang]{Geng-Rui Zhang} studies realizable and almost realizable
integer sequences, where $a_n$ counts periodic points of period dividing $n$
for a self-map \cite{Zhang2024Realizable}.  This gives a useful neighboring
dynamical viewpoint on root-of-unity and fixed-point phenomena: many classical
combinatorial sequences are realizable, while Catalan, Motzkin, and
Schröder-type sequences are not almost realizable.


\begin{proposition}
Let $\xi$ be a primitive $n^\thsup$ root of unity,
and suppose that $f \in \setN[q]$ satisfies $f(\xi^j) \in \setZ$ for all $j \in \setZ$.
...
Then it suffices to verify that, for all $d \mid n$,
\[
|\{x \in X : g^d \circ x =x \}| = f\left(\exp\left(2\pi i  \frac{d}{n}\right)\right)
\]
...
\]
Then there is a $\grpc_n$-action on some $X$ with cardinality $f(1)$
...
We can prove cyclic sieving using \hyperref[representationTheory]{representation theory}.

Suppose $G$ (in this case the cyclic group $C_n$) acts on the set $X$.
We can form the $|X|$-dimensional vector space 
\[
V = \setC X = \{a_1 x_1 + a_2x_2+\dotsb + a_M x_M : a_1,\dotsc,a_m \in \setC \},
\]
and note that $G$ acts on $V$.
We can \hyperref[computingSnCharacters]{compute} the character $\chi(g)$ for $g\in G$,
as a certain type of trace. Since $G$ in our case acts via
permutations, $\chi(g)$ is simply the number of elements in $X$ fixed by $g$.


One then wishes to find a different basis $B$ for $V$ (perhaps starting with the
...
One can also construct new CSP instances from existing ones by using representation theory,
see \cite{BergetEuReiner2011,ReinerStantonWhite2004}.
\name{David B. Rush} gives a crystal/global-basis proof of
\name{Brendon Rhoades}'s rectangular tableaux CSP theorem
\cite{Rush2021}.
\name{Young-Tak Oh} and \name{Euiyong Park} use $q$-dimensions of highest
weight crystals to obtain cyclic sieving phenomena \cite{OhPark2020x}.



\subsection[cspUniversal]{Universal CSP statistics}

The notion of universal statistics was introduced in \cite{AhlbachSwanson2018}.
...
the triple $(O,\grpc_n,\sum_{x\in O} q^{\tau(x)})$ exhibits the CSP.
...
\emph{The following definition has not been defined in any publication, to our knowledge.
This is possibly related or equivalent to the notion of a universal CSP statistic.
 --- Per}
...
Then the statistic is called \defin{proper} if, whenever $d|n$,
\[
 g^{d}(x) = x \implies   \sigma(x) \equiv_{n/d} 0   \text{ for all } x \in X.
\]

\begin{example}
Let $n=2$ and suppose $g$ generates a $\grpc_2$-action on $X$.
If $\sigma$ is a proper statistic on $X$, and $X^g \subseteq X$ is the set of
elements fixed under $g$, then there should (in principle)
...
The polynomial we use is $f_n(q) = \prod_{j=1}^{n-1}(1+q^j)$.
Then
\[
...
In \cite{Gorodetsky2019}, \name{Ofir Gorodetsky} introduces the notion of $q$-Gauß congruences.
...
\]
It can be verified that this condition on the family is equivalent to being \hyperref[lyndonCSP]{Lyndon-like},
that is $a_{n/m}(1) = a_n(\xi)$ where $\xi$ is an $n$th root of unity with order $m$,
see \cite[Cor. 2.5]{Gorodetsky2019}.
\name{Ofir Gorodetsky} also uses this criterion to prove cyclic sieving
phenomena for rotation on two word families \cite[Thm.~1.2]{Gorodetsky2019}.
The first family consists of words on $\{0,1,2\}$ with prescribed difference
between the numbers of $0$'s and $2$'s.  The second consists of binary words
with a prescribed number of $1$'s and no adjacent $1$'s cyclically; the
sieving polynomial is a refinement of the $q$-Lucas numbers.

\name{Fern Gossow} strengthens the link between $q$-Gauß congruences and
cyclic sieving \cite{Gossow2024GaussCongruence}.  The paper gives explicit
formulae for $q$-Gauß congruences and develops a universal necklace model in
which beads may vary in both colour and length, producing further CSP examples
from necklaces, path walks, tubings, and related objects.

\name{Sathyanarayan Narayan} and \name{N. Uday Kiran} study Euler--Gauß
sequences and related $q$-analogues \cite{NarayanKiran2025GaussSequences}.
Their work includes CSP conditions for $q$-Euler--Gauß sequences and for
sequences built from smallest and greatest prime factors.
...
Several examples of dihedral sieving are given in \cite{RaoSuk2020}.
\name[Matthieu Josuat-Verges]{Matthieu Josuat-Verg{\`e}s} formulates a
$q,t$-dihedral sieving phenomenon for cluster parking functions, relating
their reduced homology to \hyperref[diagonalCoinvariantRing]{diagonal
coinvariants} up to a dihedral sign character \cite{JosuatVerges2026x}.

\name{Sujit Rao} and \name{Joe Suk} have introduced the notion of \defin{$G$-sieving} for arbitrary groups.


\subsection[toricPromotionRefraction]{Toric promotion with refraction}

\name{Ashleigh Adams}, \name{Colin Defant}, and \name{Jessica Striker}
introduce toric promotion with reflections and refractions
\cite{AdamsDefantStriker2024x}.  This is a graph-labeling dynamical system
motivated by combinatorial billiards in toric arrangements.  For forests
they give a formula for the orbit structure, and for even cycles with no
reflection edges they obtain an instance of the
\hyperref[cyclicSieving]{cyclic sieving phenomenon}.


\subsection[findstatPermutationCSP]{Permutation CSP from FindStat}

\name{Ashleigh Adams}, \name{Jennifer Elder}, \name{Nadia Lafrenière},
\name{Erin McNicholas}, \name{Jessica Striker}, and \name{Amanda Welch}
give a computer-assisted analysis of cyclic sieving on permutation maps and
statistics from \href{https://www.findstat.org}{FindStat}
\cite{AdamsElderLafreniereMcNicholasStrikerWelch2025}.  They prove several
new CSP instances and record further conjectures, including examples
involving long-cycle conjugation, reverse, complement, Lehmer-code rotation,
and toric promotion.
\name{Bishal Deb} gives cyclic sieving phenomena using continued-fraction
identities for permutations, set partitions, perfect matchings, and
$D$-permutations \cite{Deb2025ContinuedFractionsCSP}.  The proofs pass
through weighted lattice paths and include involutions exchanging crossings
and nestings, including a Genocchi--Corteel involution for $D$-permutations.
\name{Mireille Bousquet-M{\'e}lou} and \name{Christian Krattenthaler} prove
cyclic sieving phenomena for corner-rooted plane trees and tree-rooted planar
maps \cite{BousquetMelouKrattenthaler2025x}.  They treat several tree classes
and root-moving cyclic actions, including actions restricted to leaves,
non-leaf corners, or corners at nodes of prescribed degree.
...
The term \defin{homomesy} was introduced by \name{Jim Propp} and
\name{Tom Roby} in \cite{ProppRoby2013x} and is defined as follows.
Let $X$ be some combinatorial set and let $\langle g \rangle$ act on $X$ such that every orbit is finite,
and let $\sigma : X \to \setZ$ be a statistic on $X$.
...
The first instance of the homomesy phenomenon was observed 
by \name{Dmitri Panyushev} in \cite{Panyushev2009},
when studying a \hyperref[rowmotion]{rowmotion}-type action on
\hyperref[antichain]{antichains} of positive roots.
Since then, many more instances have been found.


For homomesy on minuscule posets, see \cite{Okada2021},
and for the Foata map, see \cite{LaCroixRoby2020x}.
For a spectral-theory approach to homomesy, see \cite{Propp2021x}.

\name{Will Dowling} and \name{Nadia Lafrenière} carry out a systematic search
for permutation statistics that are homomesic under rotation
\cite{DowlingLafreniere2025}.  They prove several instances found through
FindStat, and show that the same homomesies hold for a broader class of
toggling actions related to \hyperref[rowmotion]{rowmotion}-style toggle
factorizations.

\name{Jennifer Elder}, \name{Nadia Lafrenière}, \name{Erin McNicholas},
\name{Jessica Striker}, and \name{Amanda Welch} systematically analyze
permutation maps and statistics in FindStat and prove $122$ homomesy
instances \cite{ElderLafreniereMcNicholasStrikerWelch2024}.  Their maps
include Lehmer-code rotation, reverse, complement, the Foata bijection, and
Kreweras complement.


An action is \defin{homometric} if the total sum of the statistic on orbits of the same size
is the same. For example, for all orbits of size 4, the sum of the statistic on each orbit must be the same.
This notion was introduced 
by \name{Sergi Elizalde}, \name{Matthew Plante}, \name{Tom Roby} and \name{Bruce Sagan}.

Homomesy and homometry for \hyperref[promotion]{promotion} on rooted trees is
considered in \cite{DangwalKimbleLiangLouSaganStewart2023}.
