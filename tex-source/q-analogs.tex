...
\metadescription{Q-analogs, q-derangement numbers, shuffle formulas, q-hook formula, q-Lucas theorem, q-Catalan numbers and q-Narayana numbers}


\section[qAnalogs]{$q$-analogs}

We use the \defin{$q$-integer}\label{qInteger},
\defin{$q$-factorial}\label{qFactorial}, and
\defin{$q$-binomial coefficient}\label{qBinomialCoefficient}:
\[
...
\]
Furthermore, for a composition $\alpha \vDash n$, the
\defin{$q$-multinomial coefficient}\label{qMultinomialCoefficient} is
\[
...
\]
These identities are true for charge and cocharge as well. Combinatorial
statistics on permutations with this distribution are called
\defin{Mahonian}\label{mahonianStatistic}.
...
This generalizes as follows. Let $W(n,\alpha)$ denote the set of words of length $n$,
...
\]
For proofs, see the lecture notes by \name{Dominique Foata} and
\name{Guo-Niu Han} \cite{FoataHan2011QSeries}.

\subsection[almkvistPartitionPolynomials]{Bounded-multiplicity partition polynomials}

The coefficient of $q^k$ in
\[
 F_{r,n}(q)=\prod_{i=1}^{n}(1+q^i+q^{2i}+\dotsb+q^{ri})
\]
counts partitions of $k$ in which each part is at most $n$ and occurs at most
$r$ times.  \name{Jianxi Mao}, \name{Wenle Shi}, and \name{Bao-Xuan Zhu} prove
Almkvist's conjecture: $F_{r,n}(q)$ is unimodal when $r$ is even, and also
when $r$ is odd and $n\geq11$ \cite[Thm.~1.3]{MaoShiZhu2026x}.

\subsection[qDerangementsAndShuffles]{$q$-derangements and shuffles}

Let $D_n \subseteq \symS_n$ be the set of derangements, that is,
permutations with no fixed points.  The \defin{$q$-derangement number}
\label{qDerangementNumber} is
\[
  d_n(q) \coloneqq \sum_{\sigma \in D_n} q^{\maj(\sigma)}.
\]
\name{Ira M. Gessel} and \name{Christophe Reutenauer} derived the following
formula from their quasisymmetric generating function for permutations with
fixed cycle structure and \hyperref[descentSet]{descent set}
\cite{GesselReutenauer1993}.  A
combinatorial proof was later given by \name{Michelle L. Wachs}
\cite{Wachs1989QDerangement}.  A recent direct proof using decorated
permutations is due to \name{Kathy Q. Ji} \cite{Ji2026x}.

\begin{theorem}[Gessel--Reutenauer--Wachs]
For $n \geq 0$,
\[
  d_n(q) =
  \sum_{k=0}^n (-1)^k q^{\binom{k}{2}}\frac{[n]_q!}{[k]_q!}.
\]
\end{theorem}

The key refinement in Wachs's proof is to count permutations by their
\defin{derangement part}\label{derangementPart}.  If
\(\pi \in \symS_n\), then \(\operatorname{dp}(\pi)\) is the reduction of
the subword of non-fixed points of \(\pi\).  For example,
\[
  \operatorname{dp}(1\,5\,3\,7\,6\,2\,9\,8\,4)=3\,5\,4\,1\,6\,2.
\]

\begin{proposition}[Wachs, \cite{Wachs1989QDerangement}]
Let \(0 \leq k \leq n\) and let \(\sigma \in D_k\).  Then
\[
  \sum_{\substack{\pi \in \symS_n\\ \operatorname{dp}(\pi)=\sigma}}
  q^{\maj(\pi)}
  =
  q^{\maj(\sigma)}\qbinom{n}{k}_q.
\]
\end{proposition}

Summing over all derangements \(\sigma \in D_k\) gives
\[
  [n]_q! = \sum_{k=0}^n \qbinom{n}{k}_q d_k(q),
\]
and the formula for \(d_n(q)\) follows by \(q\)-binomial inversion.

The shuffle identity used in this proof is due to
\name{Adriano M. Garsia} and \name{Ira M. Gessel}
\cite[Thm.~3.1]{GarsiaGessel1979}.  Let \(\alpha\) and \(\beta\) be two
words with disjoint letter sets.  A shuffle of \(\alpha\) and \(\beta\) is
an interleaving that preserves the internal order of the letters in each
word.  Write \(\operatorname{Sh}(\alpha,\beta)\) for the set of such
shuffles.

\begin{theorem}[Garsia--Gessel shuffle formula]
If \(\alpha\) and \(\beta\) have lengths \(m\) and \(n\), respectively, then
\[
  \sum_{\gamma \in \operatorname{Sh}(\alpha,\beta)} q^{\maj(\gamma)}
  =
  q^{\maj(\alpha)+\maj(\beta)}
  \qbinom{m+n}{m}_q.
\]
\end{theorem}

We define the \defin{Pochhammer symbol}\label{pochhammerSymbol} as
$(a)_n \coloneqq a(a+1)\dotsm (a+n-1)$ and the
\defin{$q$-Pochhammer symbol}\label{qPochhammerSymbol} as
\[
...
A combinatorial proof was first given by \name{Volker Strehl} \cite{Strehl1981}.
...
\name{Sara C. Billey} and \name{Joshua P. Swanson} survey
\defin{cyclotomic generating functions}\label{cyclotomicGeneratingFunction},
whose roots are roots of unity or
zero \cite{BilleySwanson2023x}.  They discuss many examples from combinatorial
representation theory, conditions for such polynomials to exist, and
asymptotic normality results for their coefficients.
...
\begin{problem}[See background in \name[I. Pak]{Igor Pak}'s survey
\emph{Combinatorial inequalities}, 2019]
...
The
\defin{MacMahon $q$-analog of the Catalan numbers}\label{macMahonQCatalan}
is the following:
\begin{align*}
...
\]
where $DP(n)$ is the set of binary words of length $2n$ with $n$ ones such
that the number of ones in any initial segment never exceeds the number of
zeros in the same segment.


The $q$-analog of \defin{Fuß--Catalan numbers}\label{qFussCatalan}
can be expressed as
\begin{align*}
...
The \defin{rational Catalan numbers}\label{rationalQCatalanNumber}
$\catalan(a/b;q)$ are defined as
\[
...
These objects admit \hyperref[cspRationalCatalan]{cyclic sieving phenomena}.



\subsection[qNarayana]{q-Narayana numbers}

The \defin{$q$-Narayana numbers}\label{qNarayanaNumber} are defined as
...
\]
where the sum is over all Dyck paths.
...
\begin{lemma}
Let $\xi$ be a primitive $d^\thsup$ root of unity with $d \mid n$ and $d>1$.
Then
\[
N(n,k;\xi) =
\begin{cases}
...
\end{lemma}
\begin{proof*}
The number $N(n,k;q)$ is equal to
 \begin{align*}
  N(n,k;q) &=
  q^{k(k - 1)- n} \left(\qbinom{n + 1}{k} \qbinom{n - 1}{k - 1}  - \qbinom{n}{k}\qbinom{n}{k - 1} \right).
\end{align*}
At $q=\xi$, $q$-Lucas tells us that at least one of the two factors in the
last term vanishes.
Hence,
\[
 N(n,k;\xi) = \xi^{k(k - 1)} \qbinom{n + 1}{k}_{\xi} \qbinom{n - 1}{k - 1}_{\xi}.
\]
Using $q$-Lucas on the first factor shows that $d\mid k$ or $d \mid (k-1)$,
or the expression vanishes.
...
Let $\lambda/\mu$ be a skew shape with $n$ boxes. Then
\begin{align*}
...
\begin{equation}
  \sum_{\sigma \in \symS_n} t^{\maj(\sigma)} q^{\maj(\sigma^{-1})}
  =
...
\begin{equation}
 \sum_{n \geq 0} \frac{z^n}{(q)_n (t)_n}
 \sum_{\pi \in \symS_n} t^{\maj(\pi)} q^{\maj(\pi^{-1})}
 =
...
\]
Here \(\charge\) denotes the \hyperref[chargeStatistic]{charge statistic}.
\end{theorem}
...
If $j\leq m$ and $m\geq k\geq 0$, then
\begin{align}
f^{(m,k)/(j)}_d(q) =
  q^{d^2} \left(
 \qbinom{m-j}{d}
...
\end{align}
\end{theorem}
The same paper shows that for $\lambda \vdash n$ and $m \geq \lambda_1$,
\begin{equation*}
...
so-called \hyperref[cspIncreasingTableau]{increasing tableaux}, introduced by
\name{Oliver Pechenik} \cite{Pechenik2014}.  In particular, \name{Xiang Chen}
provides nice \hyperref[qBinomialCoefficient]{$q$-binomial} formulas in the
case of two-row skew shapes.
...
Furthermore, let $\SYT(\alpha)$ be the set of ribbon
\hyperref[standardYoungTableau]{standard Young tableaux}
such that row $i$ from the bottom has length $\alpha_i$.
We let $D = \{s_1,\dotsc,s_\ell\}$ and $s_0 \coloneqq 0$ and note that
$s_j = \alpha_1+ \dotsb + \alpha_j$.

Then we have the following equalities.
\begin{align}
 \sum_{T \in \SYT(\alpha)} q^{\maj(T)}
 &=
 \sum_{\substack{ \sigma \in \symS_n \\ \DES(\sigma) = D }} q^{\cocharge(\sigma)}
 =
...
or \hyperref[jeuDeTaquin]{jeu de taquin}, one can show that for any
$D\subseteq [n-1]$,
\begin{equation*}
...
See also \name{Stephan Pfannerer}'s descent refinement of the
Murnaghan--Nakayama rule \cite{Pfannerer2021RefinedMN} for evaluations of
similar polynomials at roots of unity.


\begin{definition}[See \cite{Huang2020}]
 A \defin{cyclic descent map}\label{cyclicDescentMap} for $\lambda/\mu$ is a
 pair $(\cDES, \phi)$
 such that $\cDES$ sends elements in $\SYT(\lambda/\mu)$ to subsets of $[n]$,
 and $\phi : \SYT(\lambda/\mu) \to \SYT(\lambda/\mu)$ is a bijection, with
 \begin{itemize}
...
\name{Ron M. Adin} and \name{Yuval Roichman} introduce a cyclic descent
framework for involutions interpreted as matchings \cite{AdinRoichman2023}.
They construct explicit cyclic descent extensions for involutions, standard
Young tableaux, and Motzkin paths, and prove Schur positivity for the
...
\subsection[prelimEvalAtRootsOfUnity]{Evaluations at roots of unity}
...
\name{Brendon Rhoades} observed that \cite[Prop. 4.5]{Springer1974} implies the following.
...
Recall that $\chi^{\lambda}$ can be computed using the \hyperref[schurMurnaghanNakayama]{Murnaghan--Nakayama rule},
...
\]
\end{theorem}
The paper \cite{DesarmenienLeclercThibon1994} contains several evaluations of
the transformed Hall--Littlewood polynomials at roots of unity.
...
See also \cite{AyyerKumari2022,Albion2023}, where Lie group characters of
types $A$, $B$, $C$, and $D$ are evaluated at roots of unity.

\begin{theorem}[Lee--Oh 2022]
\name{So-Yeon Lee} and \name{Young-Tak Oh} prove in \cite[Thm.~16]{LeeOh2022}
the following.
Let $d \mid n$, and let $\xi$ be a primitive $d^\thsup$ root of unity.
Then $\schurS_{\lambda/\mu}(1,\xi,\xi^2,\dotsc,\xi^{n-1})$ is zero, unless
...
\end{theorem}
See also \cite{Kumari2022x} and \cite[Chap.~I, §5, Ex.~24]{Macdonald1995}.
A generalization of this identity for \hyperref[schurFlagged]{flagged Schur
polynomials} is given in \cite{Kumar2023x}.


In \cite{Albion2023}, one notes that the algebra homomorphism $\phi_r$ that sends $\completeH_j$ to $\completeH_{j/r}$
...
The above theorem by Lee--Oh can then be lifted to the symmetric functions level, see \cite{Albion2023}.
...
Recall that the $q$-Pochhammer symbol is
\[
(a;q)_n \coloneqq (1-a)(1-aq)\dotsm (1-aq^{n-1}), \qquad (a;q)_0 \coloneqq 1.
\]
This is also known as the \defin{$q$-shifted factorial}\label{qShiftedFactorial}.
Using this notation, \name{Sam Formichella} and \name{Armin Straub} extend
Gaussian binomial coefficients to negative integer arguments and prove
corresponding negation, symmetry, and Lucas-type congruence identities
\cite{FormichellaStraub2019}.
