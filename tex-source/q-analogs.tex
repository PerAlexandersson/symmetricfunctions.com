...
\]
For proofs, see \href{http://irma.math.unistra.fr/~guoniu/papers/p56lectnotes2.pdf}{these lecture notes by \name{Dominique Foata} and \name{Guo-Niu Han}}.
...
\defin{cyclotomic generating functions}, whose roots are roots of unity or
...
\end{align*}
As $q=\xi$, $q$-Lucas tells us that at least one of the two factors in the last term vanishes.
Hence,
\[
 N(n,k;\xi) = \xi^{k(k - 1)} \qbinom{n + 1}{k}_{\xi} \qbinom{n - 1}{k - 1}_{\xi}.
\]
Using $q$-Lucas on the first factor shows that $d\mid k$ or $d \mid (k-1)$ or the entire thing vanishes.
...
so called \hyperref[cspIncreasingTableau]{increasing tableaux}, introduced by \name{Oliver Pechenik} \cite{Pechenik2014}.
In particular, \name{Xiang Chen} provides nice $q$-binomial formulas in the case of two-row skew shapes.
...
or \hyperref[jeuDeTaquin]{Jeu-de-taquin}, one can show that for any $D\subseteq [n-1]$,
\begin{equation*}
...
See also \name{Stephan Pfannerer--Mittas} \href{https://www.mat.univie.ac.at/~slc/wpapers/s85vortrag/Pfannerer.pdf}{slides}
...
See also \cite{AyyerKumari2022,Albion2023}, where Lie group characters of type $A$, $B$, $C$ and $D$
