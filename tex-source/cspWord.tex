...
Let $X_n$ be the set of all \hyperref[reducedWord]{reduced words} for the longest element in the type $B_n$ \hyperref[coxeterGroups]{Coxeter group}.
We can define \hyperref[notationPermutations]{major index} on these. Let $C_{n^2}$ act on such reduced words by rotation.
Then 
\[
...
bijecting to $n\times n$-SYT and then using \hyperref[cspRectangularSYT]{\name{Brendon Rhoades} result}.
...
Let $\grpc_n$ be generated by the long cycle $(1\,2\,3\,\dotsb\,n)$, and let it act on $S_{\lambda,j}$ by conjugation.
Define 
\[
...
In \cite[Thm.~44]{AlexanderssonPfannererRubeyUhlin2020}, we show that
