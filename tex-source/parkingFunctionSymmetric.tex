...
\begin{polydata}{parking}
  ...
  Category & Other \\
  PositiveIn & powerSum | StanleyEC2 | semiring=QQ_nonnegative \\
  PositiveIn & schurS | StanleyEC2 \\
  PositiveIn & completeH | StanleyEC2 \\
\end{polydata}
...
This means (see \cite[Ex. 7.48 (f)]{StanleyEC2}) that
\begin{align*}
...
The parking function symmetric functions also occur as the path case of the
\hyperref[mobiusSymmetricFunction]{Möbius symmetric function of a graph}.
\name{Rafael S. González D'León} and \name{Michelle L. Wachs} prove
\[
  (-1)^{n-1}M_{P_n}=\omega\parking_{n-1}
\]
in \cite{DLeonWachs2026x}.


In \cite{Stanley2024x}, \name{Richard Stanley} introduces a shifted analog of $\parking_n(\xvec)$.
