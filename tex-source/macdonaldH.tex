...
It was proved by \name{Mark Haiman} in \cite{Haiman2001} that the modified Macdonald polynomials 
are \hyperref[frobeniusCharacteristic]{bigraded Frobenius characteristics} of certain $\symS_n$-modules
that appear in \hyperref[diagonalHarmonics]{diagonal harmonics}.
For details on this story, see \href{https://www.jpswanson.org/notes/n_factorial.pdf}{\name{Joshua P. Swanson}'s notes from the n! conjecture seminar}.

A combinatorial formula for the modified Macdonald polynomials was proved in \cite{HaglundHaimanLoehr2005},
where the connection with \hyperref[LLT]{LLT polynomials} gives an LLT-positive expansion.
...
It is given by
\[
\macdonaldH_{\lambda}(\xvec;q,t) = \sum_{T:\lambda \to \setP} q^{\inv_\lambda(T)} t^{\maj_\lambda(T)} \xvec^T,
\]
...
modified Macdonald polynomials is a bigraded \hyperref[frobeniusCharacteristic]{Frobenius series} with this symmetry.
However, a bijective proof is not known. 
Some partial results have been found by \name{Maria Gillespie}, \cite{Gillespie2016}.
...
In \cite{Bandlow2007}, \name{Jason Bandlow} studies more general shapes, in particular skew shapes.
