...
The \emph{ordinal sum} $P \oplus Q$ of two posets is the poset 
where 
\[
...
\]
Moreover, for the \emph{disjoint union} of posets, we have
\[
...
via the Lindström--Gessel--Viennot (LGV) lemma.

\begin{theorem}[LGV lemma, {\cite{Lindstrom1973,GesselViennot1985}}]
Let $G$ be a weighted acyclic directed graph with sources
$A_1,\dotsc,A_k$ and sinks $B_1,\dotsc,B_k$.
Let $m_{ij}$ be the total weight of paths from $A_i$ to $B_j$.
Then
\[
\det(m_{ij}) = \sum_{\sigma\in S_k} \mathrm{sgn}(\sigma)
\cdot (\text{weight of non-intersecting path systems } A_i \to B_{\sigma(i)}).
\]
In particular, if the only permutation admitting non-intersecting
paths is $\sigma = \mathrm{id}$, then $\det(m_{ij})\geq 0$
for all choices of sources and sinks, and $M$ is TNN.
\end{theorem}

A common sufficient condition is that $G$ is \emph{planar}
with sources and sinks on the boundary in a compatible order,
but planarity is not necessary---one only needs that
non-identity matchings force path intersections.

\begin{example*}[Pascal's triangle is TNN]
The matrix $M_{n,j} = \binom{n}{j}$ is TNN.
Take sources $A_n = (n,0)$ and sinks $B_j = (0,j)$
on a grid with West and North steps.
The number of paths is $\binom{n}{j}$.
For $n_1 < n_2$ and $j_1 < j_2$, a path from $A_{n_1}$
to $B_{j_2}$ (reaching a higher sink from a closer source)
must cross any path from $A_{n_2}$ to $B_{j_1}$,
since the first starts lower and ends higher.
By the LGV lemma, all minors are non-negative.
\end{example*}
...
polynomials form an interlacing sequence
$P_n \interl P_{n+1}$.
This follows from the theory of Pólya frequency sequences,
see \cite{Karlin1968}.
...
The following definitions are from \cite{Wagner1992}. See also \cite{Branden2015}
...
\subsection[bezoutMatrix]{Bézout matrices}
...
\begin{example*}[TNN for generalized tiling polynomials via LGV]
...
By the \hyperref[totallyNonnegative]{LGV lemma}, the matrix $(M_{n,j})$ is TNN,
proving that $P_n(t) = \sum_j M_{n,j} t^j$ is real-rooted and $P_n \interl P_{n+1}$.
\end{example*}
...
The descent-generating polynomial ($i$ is a descent if $i+1$ appears in a lower row):
\[
...
It can be interpreted as being a Polya frequency sequence of the Ehrhart polynomial 
implies real-rootedness of the \hyperref[ehrhart]{$h^*$-polynomial} (of a polytope).
...
\begin{theorem}[See \cite[Thm. 7]{Branden2004operators}]
...
A sequence $(a_n)_{n\geq 0}$ of non-negative reals
is a \defin{Pólya frequency sequence} (PF) if the
infinite Toeplitz matrix $T_{i,j}=a_{i-j}$ (with $a_k=0$ for $k<0$)
is totally non-negative.
By the Edrei--Thoma theorem, a sequence is PF if and only if
its generating function has the form
\[
  \sum_{n\geq 0} a_n x^n = C e^{\gamma x}
  \prod_{i} \frac{1+\alpha_i x}{1-\beta_i x}
\]
with $C,\gamma,\alpha_i,\beta_i \geq 0$ and $\sum(\alpha_i+\beta_i)<\infty$.
In particular, the coefficients of any such generating function
form a log-concave and unimodal sequence.
