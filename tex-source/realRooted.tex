...
I highly recommend \cite{Branden2015} and other works of Petter Brändén.
...
\todo{This preprint by Petter and Leonardo has lots of powerful results: https://arxiv.org/pdf/2412.06595}
...
A polynomial $Q(x)=1+a_1x+a_2x^2+\dotsb+a_n x^n$ has only negative real roots if and only if the product
\[
 \prod_{i \geq 1} Q(x_i)
\]
is Schur-positive.
...
Wagner also shows that $f \interl g$ implies that $f \interl \lambda f + \mu g \interl g$
...
A more recent result is the following theorem by Ma and Wang, \cite{MaWang2008}.
...
This result goes back to Frobenius, \cite{Frobenius1910}.

The proof goes as follows. Introduce $\tilde{A}_n(x) \coloneqq x A_n(x)$.
...
One can also consider a colored version, see Deligeorgaki, Han, Solus,
and obtain real-rooted results. MacMahon's formula above generalizes to their setting.
\end{example*}
...
real-rootedness, using the Borcea--Bränden method on stable polynomials.
...
A recursion for set partitions into blocks of size at least $s$ is also mentioned there (originally due to Comtet),
...
M. Bóna proved that these have only real zeros, \cite{Bona2009}.
...
J. Haglund and M. Visontai \cite[Thm. 3.3]{HaglundVisontai2012} show that a multivariate generalization 
...
In \cite{WangZhang2023x}, the authors consider the polynomials
\[
...
is considered by N. Amini in \cite{Amini2019x}.
...
This result conjectured by R. Stanley in \cite{Stanley1998},
a vertex-weighted version is given by A. Engström \cite{Engstrom2007}
and an alternative (non-vertex-deletion-recursive) 
proof is given by B. Lass \cite{Lass2012}.
...
Alexander Engström proved the following weighted generalization 
of the Chudnovsky--Seymour theorem. 
\begin{theorem*}[Alexander Engström, \cite[Thm. 2.5]{Engstrom2007}]
...
In \cite{BrandenVecchi2025x}, the authors show various real-rootedness results regarding Chow polynomials.
...
and it was later shown by Haglund and Zhang \cite{HaglundZhang2019} that this is indeed the case,
...
See \name{C. Athanasiadis} survey \cite{Athanasiadis2018}.
