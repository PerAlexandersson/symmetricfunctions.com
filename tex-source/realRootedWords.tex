...
\subsubsection[classicalMultisetEulerianRealRooted]{Classical and multiset Eulerian polynomials}

\begin{example*}[Eulerian polynomials, descents in permutations]\label{ex:eulerianSturm}

The \hyperref[eulerianPolynomial]{Eulerian polynomials} are
\[
...
\name{Xue Yan} subsequently proved real-rootedness for all positive
multiplicity and color vectors, without the self-interlacing restrictions
above.  Both the descent polynomial and its sentinel-ascent analogue are
real-rooted, and increasing one multiplicity gives an interlacing in each
family \cite[Thms.~1.4--1.5]{Yan2026ColoredMultiset}.
\end{example*}

\begin{example*}[Canon permutations]
A \defin{canon permutation}\label{canonPermutation} of
$\{1^k,2^k,\dotsc,n^k\}$ is a multiset permutation for which the subsequence
formed by the $j$th copy of every letter is the same permutation
$\sigma\in\symS_n$ for every $j\in[k]$.
Write
\[
 C_n^k(t)\coloneqq\sum_{\pi}t^{\des(\pi)}.
\]
Elizalde proved the factorization
\[
 C_n^k(t)=A_n(t)N_{n,k}(t),
 \qquad
 C_n^{k,\sigma}(t)=t^{\des(\sigma)}N_{n,k}(t),
\]
where $N_{n,k}(t)=t^{1-n}W_{(k^n)}(t)$ is a rectangular Narayana
polynomial \cite[Eq.~(5) and Thm.~3.1]{Elizalde2025Canon}.
Rectangular Narayana polynomials are real-rooted
\cite[Thm.~3.1]{ChenYangZhang2018Rectangular}, as are the Eulerian
polynomials.  Thus $C_n^k(t)$ and every fixed-$\sigma$ refinement
$C_n^{k,\sigma}(t)$ are real-rooted for all $n,k\geq1$.

This is separate from the colored-multiset theorem of Deligeorgaki, Han, and
Solus above.  Beck and Deligeorgaki's labeled-poset explanation and the
gamma-positivity result are discussed on the
\hyperref[gammaPositivity]{gamma-positivity page}.
The more general poset factorization there does not by itself imply
real-rootedness.  Likewise, the finer polynomials obtained by fixing a Dyck
path need not be real-rooted; see
\cite[Remark~3.3]{DeligeorgakiMenon2026x}.
\end{example*}

\subsubsection[specializedDescentPolynomialsRealRooted]{Specialized descent polynomials}

\begin{example*}[Parking-function descents]

The descent polynomial of \hyperref[parkingFunctions]{parking functions} is
\[
 D_n(t) \coloneqq \sum_{a\in\mathrm{PF}_n}t^{\des(a)}.
\]
Its coefficients form \oeis{A333829}.  For every $n\geq 1$, the polynomial
$D_n(t)$ is real-rooted.
\label{parkingFunctionDescentsRealRooted}

Indeed, set
\[
 W_{n,m}(t) \coloneqq \sum_{w\in[m]^n}t^{\des(w)}.
\]
The descent-set equidistribution of \name{Persi Diaconis} and
\name{Angela Hicks} \cite[Thm.~8]{DiaconisHicks2017} gives
\[
 D_n(t)=\frac{1}{n+1}W_{n,n+1}(t).
\]
For the word side, let $F_{r,j}(t)$ enumerate the words in $[m]^r$ ending in
$m+1-j$.  Appending the final letter gives
\[
 F_{1,j}(t)=1,\qquad
 F_{r+1,j}(t)
 =t\sum_{i<j}F_{r,i}(t)+\sum_{i\geq j}F_{r,i}(t).
\]
The coefficient matrix has $t$ below the diagonal and $1$ on and above it;
the \hyperref[matrixPreservesInterlacingSequences]{matrix interlacing
criterion} shows that $F_{r,1},\dotsc,F_{r,m}$ have a common interlacing.
The \hyperref[commonInterlacingCompatibilityTheorem]{compatibility theorem}
then implies that their sum $W_{r,m}(t)$ is real-rooted.  The transfer identity
proves the claim.

Restricting to Smirnov words, whose adjacent letters differ, instead gives the
real-rooted \hyperref[noncrossingChowPolynomial]{Chow polynomials of
...
also introduces a two-parameter Eulerian analogue with palindromicity.

For the $r=1$ specialization, Alexandersson settles the real-rootedness
question.  If $\ell\geq1$, set $E^{(\ell)}_{1,0}=1$, with entries outside the
row equal to zero, and define
\[
 E^{(\ell)}_{n,k}=(k+1)^\ell E^{(\ell)}_{n-1,k}
 +(n-k)^\ell E^{(\ell)}_{n-1,k-1},
\]
Then $E_n^{(\ell)}(t)=\sum_kE^{(\ell)}_{n,k}t^k$ has only nonpositive real
zeros and interlaces $E_{n+1}^{(\ell)}(t)$.  Its
\hyperref[gammaPolynomial]{gamma polynomial} is also real-rooted with
nonnegative coefficients \cite[Thm.~5.1]{Alexandersson2026Eulerian}.

\name{Umesh Shankar} strengthens this result by constructing real-stable
multivariate cross-step refinements for powered affine triangular recurrences
\cite[Thms.~B--C]{Shankar2026x}.  For the powered Eulerian specialization,
the zeros of \(E_n^{(\ell)}(t)\) are simple when \(n\geq2\), and the zeros of
consecutive rows strictly interlace.  The same paper interprets the
\(\ell=2\) gamma coefficients through reduced synchronized pairs of increasing
binary trees.
\end{example*}

\begin{example*}[Cyclic and even-top descents]
For northeast paths from $(0,0)$ to $(n,n)$, let $\operatorname{cdes}(w)$
count cyclic occurrences of a north step followed by an east step.  Then
\[
 C_n(t)=\sum_wt^{\operatorname{cdes}(w)}
 =2\sum_{k=1}^n\binom nk\binom{n-1}{k-1}t^k.
\]
Its coefficients are \oeis{A335340}; $C_n(t)$ has a simple zero at $0$
and $n-1$ simple negative zeros.

If $R_N(t)$ enumerates permutations by descents whose top entry is even,
then its coefficient triangle is \oeis{A134434}.  For $N\geq2$, all zeros
of $R_N(t)$ are simple and negative, and consecutive polynomials strictly
interlace \cite[Thms.~3.1 and~4.1]{Alexandersson2026Eulerian}.
\end{example*}

\begin{example*}[Rook-Eulerian polynomials]
Let $\lambda=(\lambda_1,\dotsc,\lambda_n)$ be a
\hyperref[rookPolynomial]{Ferrers board} with
$\lambda_i\geq i$, and let $Q_\lambda(t)$ enumerate row-complete nonattacking
rook placements by ascents.  The refinements by the column occupied in the
first row form an interlacing sequence, so $Q_\lambda(t)$ is real-rooted.  The
multivariate ascent-set enumerator is
\hyperref[samePhaseStable]{same-phase stable}
\cite[Thms.~12--13]{AlexanderssonJalQuemener2025x}.  For complete placements,
these objects form a lower \hyperref[bruhatOrder]{Bruhat interval} below a
$312$-avoiding permutation.
\end{example*}
...
\]
are real-rooted
\cite[Thm.~1.2]{AthanasiadisDouvropoulosKalampogiaEvangelinou2024}.  These are
...
\subsubsection[classicalTypeEulerianRealRooted]{Eulerian polynomials of classical types}
...
The analogous polynomials for type \(D\) are real-rooted, but consecutive
type \(D\) polynomials are only conjectured to form interlacing pairs; see
\cite[Section~5]{Chow2022}.

There is, however, a refined within-rank relation among the three classical
types.  Write $A_n(t)$, $B_n(t)=P_n(t)$, and $D_n(t)$ for the Eulerian
polynomials of types $A$, $B$, and $D$, respectively; in particular,
$D_n(t)$ is the \defin{type $D$ Eulerian polynomial}
\label{typeDEulerianPolynomial}; its coefficient triangle is
\oeis{A066094}.  For $n\geq 2$, Stembridge's identity is
\[
  D_n(t)=B_n(t)-n2^{n-1}tA_{n-1}(t).
\]

\begin{theorem}[Ma, \cite[Thm.~1]{Ma2026x}]
\label{halfInterlacingEulerian}
For every $n\geq 3$, the type $D$ Eulerian polynomial $D_n(t)$ has only
simple negative zeros.  Moreover, the zeros of $A_{n-1}(t)$, $B_n(t)$, and
$D_n(t)$ interlace separately on $(-\infty,-1)$ and $(-1,0)$.  The only
break from ordinary interlacing occurs across the middle at $-1$; this is
called the \defin{half interlacing property}.
\end{theorem}

The polynomial $D_2(t)=(1+t)^2$ is the exceptional repeated-root case.
When $n$ is odd, all three polynomials have the common zero $-1$.  This
within-rank half interlacing is distinct from the open consecutive
interlacing question for $D_n(t)$ and $D_{n+1}(t)$.

There is also a $q$-refinement.  For an even signed permutation
$\sigma=(\sigma_1,\dotsc,\sigma_n)$, let
\[
 \DES_D(\sigma)=
 \{0:\sigma_1+\sigma_2<0\}
 \cup\{i\in[n-1]:\sigma_i>\sigma_{i+1}\},
 \qquad
 \operatorname{neg}_D(\sigma)=|\{i\geq2:\sigma_i<0\}|,
\]
and set
\[
 D_n(t;q)=\sum_{\sigma\in D_n}
 q^{\operatorname{neg}_D(\sigma)}t^{|\DES_D(\sigma)|}.
\]
For every $n\geq2$ and $q>0$, this polynomial is real-rooted.  The proof
uses a last-entry refinement whose transition matrix preserves mutual
interlacing \cite[Thm.~3.8]{YangZhang2014x}.  The same method proves that
the affine Eulerian polynomial of every irreducible finite Weyl group is
real-rooted \cite[Thm.~4.6]{YangZhang2014x}.
\end{example*}




\subsubsection[inversionSequenceEulerianRealRooted]{Binomial and inversion-sequence Eulerian polynomials}
...
\subsubsection[peakRunPolynomialsRealRooted]{Peak and run polynomials}

\begin{example*}[Increasing runs in ternary words]
Let
\[
 W_n(t)=\sum_{w\in\{0,1,2\}^n}t^{\operatorname{run}(w)},
\]
where $\operatorname{run}(w)$ counts maximal strictly increasing consecutive
factors.  The coefficient triangle is \oeis{A120987}.  Every $W_n(t)$ is
real-rooted, and $W_n(t)$ and $W_{n+1}(t)$ weakly interlace
\cite[Thm.~6.1]{Alexandersson2026Eulerian}.
\end{example*}
...
More precisely, for $n\geq3$, $P_n$ has $\lfloor(n-1)/2\rfloor$ simple
negative zeros, and consecutive polynomials strictly alternate.  If
$\operatorname{pkv}(\sigma)$ is the set of values appearing at peaks, then
\[
 \widehat P_n(\mathbf x)=
 \sum_{\sigma\in\symS_n}\prod_{j\in\operatorname{pkv}(\sigma)}x_j
\]
is stable; its consecutive positive weighted diagonal specializations
interlace \cite[Thms.~6.2--6.3]{Alexandersson2026Eulerian}.
\end{example*}
...
\begin{example*}[Up-down runs and Stirling permutation codes]
Let $T(n,k)$ count permutations in $\symS_n$ with $k$ up-down runs after a
leading zero is adjoined; these numbers form \oeis{A186370}.  If
\[
 U_n(x)=\sum_kT(n,k)x^k, \qquad f_n(x)=\frac{2U_n(x)}x,
\]
then the \hyperref[permutationsStirling]{Stirling-permutation-code}
expansion separates the odd and even run counts as
\[
 f_n(x)=2\xi_n(x^2)+x\zeta_n(x^2),
\]
where
$\xi_n(t)=\sum_kT(n,2k+1)t^k$ and
$\zeta_n(t)=2\sum_kT(n,2k+2)t^k$.
The polynomials satisfy
\[
 f_{n+1}(x)=(1+x+(n-1)x^2)f_n(x)+x(1-x^2)f'_n(x),
 \qquad f_1(x)=2.
\]
Every zero of $f_n$ lies in $[-1,0)$, and $f_n$ weakly interlaces
$f_{n+1}$.  More precisely, $-1$ has multiplicity $\lfloor n/2\rfloor$
and the remaining $\lfloor(n-1)/2\rfloor$ zeros are simple and lie in
$(-1,0)$.  The polynomials $\xi_n$ and $\zeta_n$ also have only simple
negative zeros; $\zeta_{2m}$
\hyperref[alternatesLeftOf]{alternates left of} $\xi_{2m}$, while
$\zeta_{2m+1}$ interlaces $\xi_{2m+1}$
\cite[Thm.~6]{MaQiYehYeh2024x}.
\end{example*}


\subsubsection[restrictedPermutationDescentsRealRooted]{Descents on restricted permutations}
...
\]
These are therefore closely related to the
\hyperref[eulerianPolynomial]{classical Eulerian polynomials}.
\end{example*}


\subsubsection[derangementPolynomialsRealRooted]{Derangement polynomials}
...
The descent distribution is a different normalization.  Writing
$\mathcal{D}_n$ for the derangements in $\symS_n$, the polynomial
\[
  D_n^{\des}(t)=\sum_{\pi\in\mathcal{D}_n}t^{\des(\pi)}
\]
has only nonpositive real zeros for every $n\geq2$
\cite[Thm.~2.1]{Alexandersson2026Eulerian}.
