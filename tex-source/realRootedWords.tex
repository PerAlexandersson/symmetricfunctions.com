...
questions about \hyperref[gammaPositivity]{gamma-non-negativity} and
real-rootedness.
\end{example*}
...
In \cite{AthanasiadisEvangelinou2023xII} the authors show that for any
$T \subseteq [n-1]$,
the polynomials
\[
 A_n^T(x) \coloneqq  \sum_{\substack{\pi \in \symS_n \\ \DES(\pi) \subseteq T}} x^{\des(\pi)}
\]
are real-rooted. The proof refines the above polynomials by the value of
...
this does not follow immediately from the reference above. See also the
binomial $\svec$-Eulerian polynomials.
\end{example*}
...
See \cite{HaglundZhang2019} for the binomial Eulerian polynomial analogue of
the above results.
\end{example*}
...
$\sum_k O(M,k)x^k$ is shown to be real-rooted.
