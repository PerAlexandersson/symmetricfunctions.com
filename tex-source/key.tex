...
\begin{example*}[Worked example: $n = 3$, $\lambda = (2,1,0)$]
...
For $\lambda = (2,1,0)$, the free variables satisfy $2 \geq a \geq 1$, $1 \geq b \geq 0$,
$a \geq b$, $a \geq c \geq b$
(where $a = x_{2,1}$, $b = x_{2,2}$, $c = x_{1,1}$).
...
$L_1 + L_2$ & $s_1 s_2$ & yes & $s_1 s_2 = [3,1,2]$ \\
$L_1 + L_3$ & $s_2 s_1$ & yes & $s_2 s_1 = [2,3,1]$ \\
...
\subsubsection{Key polynomials from Kogan faces}

The key polynomial $\key_{\lambda,\sigma}$ is indexed by a partition $\lambda$
and a permutation $\sigma \in \symS_n$.
The connection to Kogan faces uses $w_0 \sigma$
(where $w_0 \sigma$ denotes the group product with the convention
$(w_0 \sigma)(i) = \sigma(w_0(i))$, i.e., $w_0$ acts first).
...
\begin{remark}
Integer points lying in multiple reduced Kogan faces
of the same type are counted \emph{once} (the definition uses a union, not a sum).
\end{remark}

\begin{example}
Continuing with $n = 3$, $\lambda = (2,1,0)$:
\begin{itemize}
\item $\sigma = \id = [1,2,3]$:
  $w_0 \sigma = w_0 = [3,2,1]$.
...
Some references define the product $w_0 \sigma$ using the opposite convention
$w_0 \circ \sigma$ (i.e., $\sigma$ acts first).
This interchanges key polynomials for $\sigma$ and $\sigma^{-1}$
(or equivalently, left and right keys in the tableaux model).
The convention used here---$(w_0 \sigma)(i) = \sigma(w_0(i))$---matches
the standard Demazure operator definition
...
\subsubsection{Kempf permutations and flagged Schur polynomials}

\begin{proposition}[Kogan {\cite{Kogan2000schubertgeometry}}]\label{prop:kempfCase}
If $\sigma \in \symS_n$ avoids the permutation pattern $132$,
...
When $w_0\sigma$ avoids $132$, the Ehrhart theory applies directly
to a single face (no union needed), making computations especially clean.


\subsubsection{Computational approach}

The $\binom{n}{2}$ left-interlacing positions in the GT diagram
form a word vector $\mathbf{w} = (w_1, \dotsc, w_{\binom{n}{2}})$
(in reading order), where each $w_k \in \{s_1, \dotsc, s_{n-1}\}$.
A Kogan face corresponds to a subset $S \subseteq \{1, \dotsc, \binom{n}{2}\}$.

\begin{enumerate}
\item \textbf{Enumerate all $2^{\binom{n}{2}}$ subsets} $S$.
\item For each $S$, extract the subword $(w_k)_{k \in S}$
  and compute the permutation $\sigma_S$ it represents.
\item \textbf{Check reducedness:} the face is reduced if and only if
  $\ell(\sigma_S) = |S|$ (the number of inversions of $\sigma_S$
  equals the length of the subword).
\item Collect all reduced faces whose type $\sigma_S$ equals $w_0 \sigma$.
\item \textbf{Enumerate integer points} in each face
  (a standard GT-pattern enumeration with forced equalities).
\item \textbf{Take the union} over all faces (remove duplicate lattice points).
\end{enumerate}
This brute-force approach is practical for $n \leq 8$ or so
($2^{\binom{8}{2}} = 2^{28} \approx 2.7 \times 10^8$);
for larger $n$, more sophisticated methods (pipe dreams, RC-graphs) are needed.

\subsubsection{Ehrhart polynomial}

Fix $\lambda$, $\sigma \in \symS_n$.
With the above theorem, it follows that the map
\[
...
For the Ehrhart polynomial approach: dilate $\lambda \to k\lambda$
and observe that $|\GT(k\lambda, \sigma) \cap \setZ^{n(n-1)/2}|$
is a polynomial in $k$.
Macdonald reciprocity provides values at negative dilations
(via strict GT-patterns with reversed inequalities),
reducing the number of evaluations needed for interpolation.
