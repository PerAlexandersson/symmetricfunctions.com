...
To define these three families, we need some additional notation.
First, let
\[
...
\]
where $(\cdot)_i$ denotes a falling factorial in the first expression.
With these definitions, we can define $\bar{\powerSum}_\gamma$ and
$\hat{\powerSum}_\gamma$, for partitions $\gamma$.
Finally, we set
\[
...
They are non-homogeneous symmetric functions and can alternatively be defined as follows.
...
surfaces.
\name{Christian Gaetz} and \name{Laura Pierson} study character polynomials
coming from moments of permutation-pattern counts
\cite[Conj.~1.4 and Thm.~1.5]{GaetzPierson2022x}.
For identity patterns, they conjecture nonnegativity of the character
coefficients and, separately, real-rootedness of the coefficient polynomials
with all roots below the pattern length.  They verify the latter for
$\lambda=\varnothing,(1),(2),(1,1)$.  The general real-rootedness conjecture
is false:
\name{Jonas Iskander} gives a degree-$11$ coefficient polynomial for the
identity pattern of length $14$ which has only nine real roots
\cite[Ex.~1.1]{Iskander2023x}.
...
Here,
$A_{\lambda,\mu}
 = \langle \schurS_\lambda,
 \schurS_\mu[1 + \completeH_1 + \completeH_2 + \dotsb] \rangle$,
