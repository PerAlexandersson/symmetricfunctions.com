...
\metadescription{A short guide to algebraic varieties appearing in algebraic combinatorics, including Grassmannians, flag varieties, Schubert varieties, Schubert matroids, Richardson varieties, Springer fibers, Peterson varieties, Hessenberg varieties, and Demazure characters.}
...
The main examples are \hyperref[grassmannian]{Grassmannians},
\hyperref[flagVarieties]{flag varieties}, Schubert varieties,
\hyperref[richardsonVarieties]{Richardson varieties},
\hyperref[springerFibers]{Springer fibers}, and
\hyperref[hessenbergVarieties]{Hessenberg varieties}.  They connect directly
to \hyperref[schubertCalculus]{Schubert calculus},
...
An \defin{affine variety} is, roughly, the set of common zeros of polynomial
equations in affine space:
\[
  X=\{x\in\setC^n:f_1(x)=\dotsb=f_r(x)=0\}.
\]
A \defin{projective variety} is the corresponding notion in projective space,
where points are lines through the origin:
\[
  \mathbb P^{n-1}
  =
...
The \defin{Grassmannian} $\operatorname{Gr}(k,n)$ is the variety of
$k$-dimensional subspaces of $\setC^n$.  Equivalently, it parametrizes
projective $(k-1)$-planes in $\mathbb P^{n-1}$.  For example,
$\operatorname{Gr}(2,4)$ parametrizes lines in $\mathbb P^3$.

The standard projective realization is the \defin{Plücker embedding}
\[
...
minors are the \defin{Plücker coordinates}.  These coordinates satisfy
quadratic Plücker relations.

This is where matroids enter.  Given a point of $\operatorname{Gr}(k,n)$, the
sets $I\subseteq[n]$ with nonzero Plücker coordinate $p_I$ form the bases of a
rank $k$ representable matroid.  The totally nonnegative part of the
...
lattice path matroids are important positroid families \cite{Oh2011}.


\section[flagVarieties]{Flag varieties}

A \defin{complete flag} in $\setC^n$ is a chain
\[
  0=F_0\subset F_1\subset F_2\subset\dotsb\subset F_n=\setC^n,
  \qquad \dim F_i=i.
\]
The \defin{flag variety} $\operatorname{Fl}_n$ is the variety of all complete
flags.  A \defin{partial flag variety} remembers only some of the subspaces in
the chain.  Grassmannians are partial flag varieties, since
...
Fix a complete flag $F_\bullet$ in $\setC^n$.  Schubert varieties are
...
\defin{Schubert matroid} has bases
\[
  \mathcal B_I
  =
  \{J=\{j_1<\dotsb<j_k\}: j_a\leq i_a\text{ for all }a\}.
\]
For example, if $I=\{2,4\}$, then
\[
  \mathcal B_I=\{12,13,14,23,24\}.
\]
...
A \defin{Richardson variety} is an intersection of a Schubert variety and an
opposite Schubert variety.  In the complete flag variety, if $X_w$ is the
Schubert variety indexed by $w$ and $X^v$ is the opposite Schubert variety
indexed by $v$, then
\[
  X_w^v\coloneqq X_w\cap X^v.
\]
This intersection is nonempty precisely when $v\leq w$ in Bruhat order.

Richardson varieties are useful because they isolate the geometry of an
interval in Bruhat order.  They also appear naturally in total positivity,
...
Hessenberg varieties are subvarieties of the flag variety cut out by a linear
operator and a Hessenberg function.  In type $A$, one starts with a weakly
increasing sequence $m=(m_1,\dotsc,m_n)$ with $i\leq m_i\leq n$ and considers
flags satisfying
\[
...
A \defin{Springer fiber} is a subvariety of the flag variety attached to a
nilpotent linear operator $N$.  In type $A$, it is
\[
  \mathcal B_N
  =
...
this is the starting point of Springer theory \cite{Springer1974}.  Springer
...
The \defin{Peterson variety} is a special regular nilpotent Hessenberg
variety.  In type $A$, it is obtained from a regular nilpotent operator $N$
and the Hessenberg function
\[
  m(i)=i+1\quad(1\leq i<n),\qquad m(n)=n.
\]
Thus its flags satisfy
\[
  N F_i\subseteq F_{i+1}.
\]
...
The subgroup $B$ is a Borel subgroup, and its stable subvarieties inside
$G/B$ are precisely Schubert varieties.
...
\defin{Demazure character}, also called a key polynomial
...
vexillary Schubert polynomials are flagged Schur functions.
