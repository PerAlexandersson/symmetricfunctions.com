...
Holmes gives an efficient recursion for irreducible symmetric-group character
values \cite{Holmes2017x}.
\name{Alexander R. Miller} uses a reformulation of the
\hyperref[murnaghanNakayamaRule]{Murnaghan--Nakayama rule} to study
congruences in symmetric-group character tables \cite{Miller2019x}.
\name{Timothy Y. Chow} and \name{Jennifer Paulhus} study algorithms for
distinguishing irreducible characters of the symmetric group
\cite{ChowPaulhus2021}.


\subsection[borderStripTableauxDefinition]{Border strip tableaux}

A \defin{border strip tableau} (also known as \defin{ribbon tableau} or \defin{rim-hook tableau})
...
We let $\ht(B)$ denote the height of $B$.
...
\name{Yinsong Chen} and \name{Vladislav Kargin} study ribbon tilings of large
rectangles and fixed-width strips \cite{ChenKargin2023x}.  For order-$n$
ribbons they prove the existence of a rectangle growth rate, bound it by
$(n-1)\log 2$, and derive asymptotic bounds for strip growth rates.  They also
give recursive systems that enumerate order-$n$ ribbon tilings of strips for
$n\leq 8$ and produce the corresponding generating functions.
...
This definition is from \cite{Oguz2020}; see \cite{Descouens2008} for an alternative
way to phrase this.
...
Note that for $k=1$ we recover the set of
\hyperref[semistandardYoungTableau]{semistandard Young tableaux}, and the
\hyperref[kostkaCoefficient]{Kostka coefficients}.

\name{Florence Maas-Gariépy} and \name{Rebecca Patrias} introduce
set-valued domino tableaux and shifted set-valued domino tableaux
\cite{MaasGariepyPatrias2020}.  These are $K$-theoretic analogues of domino
tableaux and shifted domino tableaux.
\name{Dina Abbasian}, \name{Lena Difulvio}, \name{Robert Muth},
\name{Gabrielle Pasternak}, \name{Isabella Sholtes}, and
\name{Frances Sinclair} study cuspidal ribbon tableaux in affine type $A$
\cite{AbbasianDifulvioMuthPasternakSholtesSinclair2023}.
\name{Ekaterina A. Vassilieva} studies products of generating functions for
domino tableaux and bi-tableaux \cite{Vassilieva2019x}.
...
\begin{figure}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-1.svg}{The first of six 3-ribbon tableaux with shape 4422 and weight 1111.}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-2.svg}{The second of six 3-ribbon tableaux with shape 4422 and weight 1111.}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-3.svg}{The third of six 3-ribbon tableaux with shape 4422 and weight 1111.}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-4.svg}{The fourth six 3-ribbon tableaux with shape 4422 and weight 1111.}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-5.svg}{The fifth six 3-ribbon tableaux with shape 4422 and weight 1111.}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-6.svg}{The last of six 3-ribbon tableaux with shape 4422 and weight 1111.}
\end{figure}

The following figure shows the elements in $\SSYT^{(3)}(4422,211)$.
\begin{figure}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-7.svg}{The first of the three 3-ribbon tableaux with shape 4422 and weight 211.}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-8.svg}{The second of the three 3-ribbon tableaux with shape 4422 and weight 211.}
  \svgimg[width=0.1\textwidth]{svg-images/ribbon-tableau-9.svg}{The last of the three 3-ribbon tableaux with shape 4422 and weight 211.}
\end{figure}
\end{example*}


The notion of $k$-ribbon tableaux appears frequently when studying \hyperref[plethysm]{plethysm} coefficients,
\hyperref[prelimEvalAtRootsOfUnity]{evaluations at roots of unity}, and in
the study of LLT polynomials.
...
See also the \href{https://ghseeli.github.io/grad-school-writings/class-notes/algebraic-combinatorics.pdf}{lecture notes by \name{George Seelinger} (taught by \name{Jennifer Morse})}.
...
and the vertical step in the same column are both labeled $j$.
...
a \hyperref[prelimPartitions]{Frobenius coordinate} perspective.
...
to an $\ell$-tuple $(T_0,\dotsc,T_{\ell-1})$ of standard Young tableaux,
but where the entries $1,2\dotsc,$ are distributed among the tableaux.
...
\begin{figure}
\svgimg[width=0.25\textwidth]{svg-images/border-strip-tableau.svg}{A border-strip tableau of shape 8766641 with 3-strips.}
\begin{ytableau}
1 & 3 & 8 \\
2 & 11 & 12\\
\end{ytableau}
...
\bottomrule
\end{tabular}
Note that \name{Igor Pak} has a different labeling convention, so one 
...
In other words, the \hyperref[schurMurnaghanNakayama]{Murnaghan--Nakayama rule} is cancellation-free.
